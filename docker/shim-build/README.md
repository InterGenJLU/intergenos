...
2. **Multi-host verification: COMPLETE AS-OF 2026-05-21.** Original
   ...
   re-attestation on Build Host A produced tarball sha256
   ...
   Cross-host re-attestation completed 2026-05-21 on Build Host C
   (`ubuntu-hplt`; Ubuntu 24.04 + Docker 29.4.3; independent witness) —
   produces byte-identical SHAs. Docker-major-version diversity (27.x
   on Host A vs 29.x on Host C) is the substantive cross-host
   attestation property. Log file
   `~/intergenos/shim-review-evidence/host-c-verify-20260521.log`
   (9-check `verify-b2-reproducibility.sh` format) is staged for
   inclusion in the shim-review fork `logs/` directory at PR-open time.
   ...
   evidence. **Note:** Docker Desktop (Windows/macOS) is excluded from
   this attestation surface — its virtualization layer produces
   internally-consistent-but-divergent SHAs. Reproducibility attestation
   requires native-Linux Docker.
