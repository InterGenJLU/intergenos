...
def run_post_install_hooks(target, packages_dir, installed_names=None,
                           progress_callback=None):
    """Run post_install() hooks for the installed packages that have them.
    ...
    Args:
        ...
        installed_names: if given, the resolved install-set name list; only
            packages whose archive-name resolves into this set have their
            post_install run. Without it (None) EVERY package in the tree runs
            -- including ones the user did NOT install, whose post_install then
            side-effects the target for absent software (orphan /etc dirs, a
            stray self-signed cert, failed chowns). Pass the install set so a
            deselected package's hook never fires.
        progress_callback: fn(current, total, name) called per hook
    """
    target = Path(target)
    packages_dir = Path(packages_dir)

    installed_set = set(installed_names) if installed_names is not None else None
    if installed_set is not None:
        from .packages import _archive_name_candidates
    ...
    for tier_dir in sorted(packages_dir.iterdir()):
        ...
        for pkg_dir in sorted(tier_dir.iterdir()):
            if not pkg_dir.is_dir():
                continue
            # Skip packages that were not installed. The install set uses
            # archive names, which can differ from the source dir name via the
            # `-core` convention, so resolve every candidate.
            if installed_set is not None and not any(
                cand in installed_set
                for cand in _archive_name_candidates(pkg_dir.name)
            ):
                continue
    ...
    try:
        ...
        trace.trace_event(
            "recipe_hooks_skipped", total=total, reason=str(e),
        )
    ...
    try:
        for i, hook in enumerate(hooks, 1):
            ...
            rc, stdout, stderr = trace.traced_run_chroot(
                target, cmd, phase="hooks", pkg=hook["name"],
                intent=f"recipe post_install hook for {hook['name']}",
            )
            if rc == 0:
                executed += 1
                log.debug("post_install hook %s: rc=0", hook["name"])
            else:
                failed.append(hook["name"])
                log.warning(
                    "post_install hook %s exited %d; install proceeds "
                    "(hook errors are non-fatal — some hooks expect "
                    "services that aren't running yet)\n"
                    "stdout: %s\nstderr: %s",
                    hook["name"], rc, stdout.strip(), stderr.strip(),
                )
    ...
    log.info(
        "post_install hooks: %d/%d succeeded%s",
        executed, total,
        f"; failed: {', '.join(failed)}" if failed else "",
    )
    trace.trace_event(
        "recipe_hooks_summary",
