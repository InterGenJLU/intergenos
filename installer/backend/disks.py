...
# Decided 2026-08-11: the two EXPERIMENTAL LUKS unlock methods (TPM2-sealed key,
# FIDO2 token) are NOT OFFERED by the installer in this release. An external
# functionality review (2026-08-08) found both not functional as shipped: FIDO2
# enrollment aborts an install that opts in, and the TPM2 sealed key's parent
# context cannot survive a reboot, so auto-unlock never survives the first
# power cycle. Every boot-side failure already falls through to the passphrase
# prompt, so withholding the switches removes a broken promise and nothing
# else. The enrollment plumbing below is retained deliberately; it is re-offered
# only after the reviewed corrections land with dedicated hardware verification
# (enroll + reboot on real TPM2 and FIDO2 hardware).
EXPERIMENTAL_UNLOCK_OFFERED = False
...
# Mountpoint prefixes that indicate a disk is hosting the live-ISO
# media. Adding the USB stick to the install picker would be a foot-gun:
# the user picks their install USB instead of the internal disk and
# the install kicks off destroying the live media it's running from.
# Cross-distro coverage:
#   /run/iso          — InterGenOS live ISO (observed 2026-05-26 on
#                       intergenos-1.0-dev1.iso boot)
#   /run/squashfs     — squashfs loopback mount on the same live boot
#   /run/live         — Debian-live convention
#   /run/initramfs/live — Fedora live convention
#   /cdrom            — Ubuntu / Casper convention
#   /media/cdrom      — generic
# Also: any partition with fstype 'iso9660' or 'udf' is a live medium
# regardless of mountpoint.
_LIVE_MEDIA_MOUNT_PREFIXES = (
    "/run/iso",
    "/run/squashfs",
    "/run/live",
    "/run/initramfs/live",
...
_LIVE_MEDIA_FSTYPES = ("iso9660", "udf")


def _lsblk_device_fields(dev_path):
    """Read PKNAME + TRAN for one device via `lsblk -Pdno` key="value" pairs.

    The -P pairs form is used (not -r raw) because PKNAME is legitimately
    empty for a whole-disk device and raw output would collapse the empty
    ...
    if result.returncode != 0 or not result.stdout.strip():
        return {}
    fields = {}
    for match in re.finditer(r'(\w+)="([^"]*)"', result.stdout.splitlines()[0]):
        fields[match.group(1)] = match.group(2)
    return fields


def live_media_kind():
    """Classify the device hosting the live session's install media.

    Returns:
        "cdrom" — an optical device (`sr*`/`scd*`), which is how a VM's
                  virtual CD/DVD drive presents regardless of bus.
        "usb"   — a USB mass-storage device (transport `usb`, resolved on
                  the partition's parent disk when needed).
        None    — no live-media mount found, or classification failed.

    Feeds user-facing guidance wording ONLY (which removal instruction the
    completion screens print) — never a partitioning decision — so every
    error path deliberately returns None and callers fall back to the
    generic "install media" phrasing. The mount prefixes are the same
    cross-distro set detect_disks() uses to exclude the live medium from
    the install picker.
    """
    for prefix in _LIVE_MEDIA_MOUNT_PREFIXES:
        ...
        if not source.startswith("/dev/"):
            # Overlay/loop pseudo-sources (e.g. the squashfs loop) say
            # nothing about the physical medium; keep scanning prefixes.
            continue
        name = os.path.basename(source)
        if name.startswith(("loop", "dm-")):
            # A loopback/device-mapper source (e.g. the squashfs loop)
            # says nothing about the physical medium; keep scanning.
            continue
        if name.startswith(("sr", "scd")):
            return "cdrom"
        fields = _lsblk_device_fields(source)
        transport = fields.get("TRAN", "")
        if not transport and fields.get("PKNAME"):
            # Partition rows carry no transport; ask the parent disk.
            parent = _lsblk_device_fields("/dev/" + fields["PKNAME"])
            transport = parent.get("TRAN", "")
        if transport == "usb":
            return "usb"
        return None
    return None
...
def detect_disks():
    ...
    the installation media itself, and the disk hosting the live ISO
    (matched by partition mountpoints + fstypes — see
    `_LIVE_MEDIA_MOUNT_PREFIXES` / `_LIVE_MEDIA_FSTYPES`).
    """
    ...
    for dev in data.get("blockdevices", []):
        ...
        is_root_disk = False
        is_live_media = False
        # Whole-disk fstype check (some live USBs are iso9660 written
        # directly to the disk with no partition table).
        if (dev.get("fstype") or "") in _LIVE_MEDIA_FSTYPES:
            is_live_media = True
        ...
        for child in dev.get("children", []):
            if child.get("type") == "part":
                ...
                if mountpoint == "/":
                    is_root_disk = True
                # Exclude disks hosting the live ISO. Partition mount-
                # point matches the live-media prefixes (the squashfs
                # loopback's source partition lives here) OR partition
                # fstype is iso9660 / udf (the ISO partition itself).
                if any(mountpoint.startswith(p) for p in _LIVE_MEDIA_MOUNT_PREFIXES):
                    is_live_media = True
                if fstype in _LIVE_MEDIA_FSTYPES:
                    is_live_media = True

        if is_root_disk or is_live_media:
            continue
...
def _release_disk(disk_path):
    """Best-effort release of every holder on the target disk so wipefs +
    repartition can take exclusive ownership.

    Reinstalling over a prior LVM / LUKS / RAID Linux install leaves active
    device-mapper holders (e.g. an active LVM volume group) that pin the disk
    busy; without releasing them `wipefs -a` fails with EBUSY. (GBC001.5 first
    bare-metal install: the target carried an active `ubuntu-vg` LVM LV ->
    `wipefs -a /dev/sda: probing initialization failed: Device or resource
    busy`.)

    Order: swapoff -> unmount -> deactivate LVM VGs whose PVs are on this disk
    -> tear down any remaining dm holders (deepest first) -> udev settle. Every
    step is best-effort and tolerates 'nothing to do' / a missing tool; what
    matters is the disk ending up free, not any single command succeeding.

    SAFETY: only holders that descend from `disk_path` are touched (enumerated
    via `lsblk <disk_path>`). The live ISO's own dm-verity root (igos-root,
    backed by loop devices, not this disk) never appears in that tree and is
    never disturbed.
    """
    if _DRY_RUN:
        print(f"  [DRY-RUN] release holders on {disk_path} "
              f"(swapoff / umount / vgchange -an / dmsetup remove)")
        return

    import os.path as _op

    def _best_effort(cmd):
        # Releasing holders is genuinely best-effort: many of these commands
        # legitimately "fail" (no swap to swapoff, nothing mounted, no LVM) and
        # that is fine. But swallowing EVERYTHING silently meant a real release
        # failure that later surfaces as a cryptic `wipefs: Device or resource
        ...
        try:
            ...
            LOG.debug("release best-effort %s raised %s: %s",
                      cmd, type(e).__name__, e)

    def _node(name):
        if name.startswith("/dev/"):
            return name
        if _op.exists(f"/dev/{name}"):
            return f"/dev/{name}"
    ...
    # 1. swapoff + unmount active/mounted nodes (deepest child first).
    for name, _typ, mnt in reversed(rows):
        if mnt == "[SWAP]":
            _best_effort(["swapoff", _node(name)])
        elif mnt:
            _best_effort(["umount", mnt])

    # 2. Deactivate LVM volume groups whose PVs sit on this disk (leaves any
    #    other VG -- and the live verity root -- untouched).
    vgs = []
    for name, _typ, _mnt in rows:
        try:
            ...
            out = ""
        for vg in out.split():
            if vg and vg not in vgs:
                vgs.append(vg)
    for vg in vgs:
        _best_effort(["vgchange", "-an", vg])

    # 3. Tear down any remaining dm holders layered on the disk (deepest
    #    first) -- catches LUKS / RAID / leftover mappings vgchange missed.
    for name, typ, _mnt in reversed(rows):
        if typ in ("lvm", "crypt", "dm", "mpath",
                   "raid0", "raid1", "raid4", "raid5", "raid6", "raid10"):
            _best_effort(["dmsetup", "remove", "--force", name])

    # 4. Let udev catch up so the subsequent wipefs/parted see a quiet disk.
    _best_effort(["udevadm", "settle"])
...
def partition_disk(disk_path, efi=False, luks_enabled=False, luks_passphrase=None,
    ...
    # Release any active holders (LVM VG, LUKS, RAID, mounts, swap) layered on
    # the target before wiping. Reinstalling over a prior LVM/LUKS install
    # otherwise fails on the next line with `wipefs ... Device or resource
    # busy` (GBC001.5 first bare-metal install). Best-effort; see _release_disk.
    _release_disk(disk_path)
    ...
    if efi:
        ...
        # Settle udev before touching the new partition nodes: parted writes the
        # table, but /dev/<disk>p1,p2 are (re)created asynchronously by udev, so
        # an immediate mkfs/luksFormat can race a not-yet-present node and abort
        # the install AFTER the table is written (disk wiped, no system). The
        # alongside path already guards this; the primary fresh-install path did
        # not.
        _run(f"partprobe {disk_path}")
        _run("udevadm settle")
        ...
        # Settle udev before touching the new partition nodes (see the EFI
        # branch above): avoids the mkfs-races-udev abort after the table is
        # written.
        _run(f"partprobe {disk_path}")
        _run("udevadm settle")
...
def luks2_format(partition_path, passphrase):
    ...
    Secret-handling discipline: passphrase never reaches argv, never goes to
...
def tpm2_seal_random_key(intergen_dir, luks_partition, luks_passphrase):
    ...
    Secret-handling discipline: random_secret is bytes from os.urandom in
...
def _luks_add_key_with_existing(luks_partition, existing_key, new_key):
    ...
    path to cryptsetup, feed existing_key via stdin. Secret-handling: new_key
