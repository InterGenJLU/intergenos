...
# ---- Bake /etc/crypttab so fde-init.sh can resolve the LUKS device (Gap B) --
# fde-init.sh's PRIMARY device-identity source is /etc/crypttab: it awk's field 2
# for the `cryptroot` mapping (a stable `UUID=<luks-uuid>` line Forge writes via
# config.generate_crypttab) and field 4 for the tpm2/fido2 EXPERIMENTAL options.
# There is no /etc on the not-yet-mounted encrypted root when the initramfs runs,
# so the crypttab must be BAKED into the cpio. This builder runs in the target
# context (build-time via chroot-build-bootloader.sh, runtime via the kernel
# post-install hook), where /etc/crypttab is present on a LUKS install. The
# kernel `cryptdev=` cmdline is fde-init.sh's fallback when this is absent (a
# plain install, or a build-time bake before the install wrote crypttab).
CRYPTTAB_SRC="${CRYPTTAB_SRC:-/etc/crypttab}"
if [ -f "$CRYPTTAB_SRC" ]; then
    cp "$CRYPTTAB_SRC" "$WORK/etc/crypttab"
    chmod 0600 "$WORK/etc/crypttab"
    echo "  Gap B: baked $CRYPTTAB_SRC into the FDE initramfs (device identity)"
else
    echo "  Gap B: $CRYPTTAB_SRC absent at build time — fde-init.sh will resolve"
    echo "         the LUKS device from the cryptdev= kernel cmdline instead"
fi
...
if [ "$HAVE_TPM2_TOOLS" = "yes" ]; then
    ...
    # tpm2-tools 5.x ships as a multicall binary: one real `tpm2` executable
    # plus tpm2_<verb> symlinks. Mirror the on-disk layout (1 real binary +
    # 4 symlinks) inside the initramfs to avoid 5x duplication of the
    # ~10MB binary. cp WITHOUT -P/-d follows symlinks, so a naive per-verb
    # cp loop would produce 5 full copies — preserve the multicall layout
    # explicitly.
    if [ -x "$TPM2_TOOLS_DIR/tpm2" ]; then
        cp "$TPM2_TOOLS_DIR/tpm2" "$WORK/sbin/tpm2"
        chmod +x "$WORK/sbin/tpm2"
        ln -sf "/sbin/tpm2" "$WORK/bin/tpm2"
        for verb in tpm2_createprimary tpm2_create tpm2_load tpm2_unseal tpm2_flushcontext; do
            ln -sf tpm2 "$WORK/sbin/$verb"
            ln -sf "/sbin/$verb" "$WORK/bin/$verb"
        done
    else
        echo "  WARNING: $TPM2_TOOLS_DIR/tpm2 multicall binary missing — TPM2 unlock NOT bundled"
        HAVE_TPM2_TOOLS="no"
    fi
else
...
if [ "$HAVE_FIDO2_TOOLS" = "yes" ] || [ "$HAVE_TPM2_TOOLS" = "yes" ]; then
    mkdir -p "$WORK/usr/lib" "$WORK/lib64"
    # Both fido2-tools-static AND tpm2-tools-static are mostly-static (per
    # D-001 D-OPTION-A 2026-05-23). FIDO2 binaries link libudev/libc/ld-linux
    # dynamically; TPM2 multicall binary additionally links the libcurl chain
    # (libcurl + libnghttp2 + libidn2 + libunistring + libssh2 + libpsl) plus
    # libssl + libcrypto + libz + libzstd via the multicall design that
    # bundles all ~70 verbs in one executable (tpm2_getekcertificate + FAPI
    # tools are unconditional even though we don't use them — empirically
    # verified --disable-fapi did NOT drop the curl chain). The ldd loop
    # below catches the full set for both binary families.
    ...
    # systemd-adds-deps cases without changing the script. Iterates BOTH
    # fido2 binaries AND the tpm2 multicall binary — tpm2's libcurl chain
    # gets bundled here (~10 .so files when TPM2 unlock is enrolled).
    BUNDLE_BINARIES=""
    if [ "$HAVE_FIDO2_TOOLS" = "yes" ]; then
        BUNDLE_BINARIES="$BUNDLE_BINARIES fido2-cred fido2-assert fido2-token"
    fi
    if [ "$HAVE_TPM2_TOOLS" = "yes" ]; then
        BUNDLE_BINARIES="$BUNDLE_BINARIES tpm2"
    fi
    for bin_name in $BUNDLE_BINARIES; do
        [ -x "$WORK/sbin/$bin_name" ] || continue
        ldd "$WORK/sbin/$bin_name" 2>/dev/null | awk '/=>/ && $3 ~ /^\/.*\.so/ { print $3 }' | while read -r so; do
            so_name="$(basename "$so")"
            ...
            echo "  D-001/I-D: bundled $so_name (transitive dep of $bin_name)"
        done
    done
fi
...
#   nvme         — NVMe SSDs (most modern laptops, incl. the HP test fleet)
