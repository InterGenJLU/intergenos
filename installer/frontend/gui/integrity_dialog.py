...
def open_integrity_dialog_async(
    parent_window, package_name, expected_sha256, actual_sha256, on_ack,
):
    """Architecture-B async equivalent of the make_gui_integrity_callbacks
    pair. Routes through the D-Bus ack callback the caller supplies
    (`on_ack(override_phrase: str) -> bool`).

    Flow:
      1. Show the warning dialog (modal, single-button).
      2. On warning dismiss, show the typed-phrase override dialog.
      3. On Submit (text matches expected), invoke `on_ack(phrase)` —
         which calls AckIntegrityOverride over D-Bus and returns the
         backend's accepted/rejected verdict. The backend's worker
         thread is blocked waiting on this.

    All dialogs run on the GLib main loop — no blocking, no worker
    thread needed. Suitable for invocation from a D-Bus signal handler.
    """
    expected_phrase = expected_override_phrase(package_name)

    warning = Adw.MessageDialog(
        transient_for=parent_window,
        modal=True,
        heading=f"INTEGRITY WARNING — {package_name}",
        body=_build_warning_text(package_name, expected_sha256, actual_sha256),
    )
    warning.add_response("ack", "Continue")
    warning.set_default_response("ack")
    warning.set_close_response("ack")

    def _on_warning_dismissed(_dialog, _response):
        _show_ack_dialog_async(parent_window, package_name, expected_phrase, on_ack)

    warning.connect("response", _on_warning_dismissed)
    warning.present()


def _show_ack_dialog_async(parent_window, package_name, expected_phrase, on_ack):
    dialog = Adw.MessageDialog(
        transient_for=parent_window,
        modal=True,
        heading=f"Override integrity check for {package_name}?",
    ...
    entry = _make_paste_disabled_entry(expected_phrase, _on_match)
    dialog.set_extra_child(entry)

    def _on_response(d, response):
        phrase = entry.get_text() if response == "submit" else ""
        d.close()
        # Submit to backend via D-Bus (on_ack returns the backend's verdict).
        # We don't block here; on_ack synchronously calls D-Bus method.
        on_ack(phrase)
