...
class DiskPage(_ForgePage):
    ...
    def _build_body(self) -> Gtk.Widget:
        ...
        # A second way into the disk, offered on every encrypted install:
        # one key slot and a forgotten passphrase is a disk nobody can open.
        # Not experimental and not hardware-dependent, so it sits above the
        # two experimental rows and is offered whenever encryption is on.
        self._recovery_key_switch = Adw.SwitchRow()
        self._recovery_key_switch.set_title("Add a recovery key")
        self._recovery_key_switch.set_subtitle(
            "A second way to unlock this disk, generated by this computer "
            "and shown once when the install finishes. Write it down then — "
            "it is not saved on the disk or in the install record."
        )
        self._recovery_key_switch.set_visible(False)
        self._enc_group.add(self._recovery_key_switch)
    ...
    def _on_luks_toggled(self, switch_row, _pspec):
        ...
        self._recovery_key_switch.set_visible(active)
        from installer.backend import disks as _disks
        ...
        if not active:
            ...
            self._luks_strength_row.set_visible(False)
            self._recovery_key_switch.set_active(False)
    ...
    def on_next(self, state):
        ...
        state.luks_recovery_key_enable = bool(
            luks_enabled and self._recovery_key_switch.get_active())
