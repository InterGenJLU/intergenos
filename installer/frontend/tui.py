...
def prompt_install_io():
    ...
    if luks_enabled:
        ...
        # The second way in. One key slot and a forgotten passphrase is a
        # disk nobody can open, so the person is offered a recovery key the
        # machine generates and shows once at the end of the install.
        recovery_key_enable = _ask_yesno(
            "Add a recovery key",
            "Your passphrase will be the only way to unlock this disk.\n"
            "If it is forgotten or mistyped into a different keyboard\n"
            "layout, the data is gone — nobody can recover it.\n\n"
            "Add a recovery key as a second way in? It is generated by\n"
            "this machine and shown once when the install finishes, so\n"
            "have something to write it down with. It is never stored\n"
            "anywhere: not on this disk, not in the install record.",
        )
    ...
    if luks_enabled:
        ...
        if recovery_key_enable:
            install_io["luks_recovery_key_enable"] = True
        if tpm2_enabled:
...
def run_declarative(yaml_path, install_io, archive_dir, packages_dir, dry_run):
    ...
    if result.success:
        ...
        if result.luks_recovery_key:
            # The one and only time this is shown. It is not in the install
            # record and not on the disk, so a person who does not write it
            # down now has lost it — say that, rather than letting them
            # assume it can be looked up later.
            _reporter.info("")
            _reporter.note("RECOVERY KEY for the encrypted disk — write this down now:")
            _reporter.info("")
            _reporter.info(f"    {result.luks_recovery_key}")
            _reporter.info("")
            _reporter.detail(
                "This is the only time it is shown. It is not saved on the "
                "disk or in the install record. Keep it somewhere that is "
                "not this machine; anyone holding it can unlock this disk.")
            _reporter.info("")
