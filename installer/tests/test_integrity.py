...
class TestSignatureVerification(unittest.TestCase):
    """verify_manifest_signature() is the install-integrity trust anchor. The
    hardened contract (integrity.py, 2026-06-17) requires it to:
      (a) FAIL CLOSED on a missing .sig (absent signature is not a skip);
      (b) verify with `gpgv` (NOT `gpg --keyring`, which gpg 2.5/keyboxd
          silently ignores), after `gpg --dearmor`-ing the armored key;
      (c) PIN the primary-key fingerprint — a clean exit alone accepts whatever
          key ships beside the manifest, so a full-media-rewrite attacker who
          ships their own {manifest, sig, key} verifies green unless we anchor
          on the in-code RELEASE_MASTER_FPR.
    The hardened path makes TWO subprocess calls (gpg --dearmor, then gpgv);
    these tests mock that pair and assert each property. (The prior tests
    predated the hardening: setUp created no .sig, so every case short-circuited
    at the fail-closed check before any subprocess ran — they exercised none of
    the above. Rewritten to lock the hardened contract in.)"""

    def setUp(self):
        self.tmp = Path(tempfile.mkdtemp())
        self.manifest = self.tmp / "manifest.txt"
        self.manifest.write_text("# fake manifest\n")
        self.sig = Path(str(self.manifest) + ".sig")
        self.sig.write_text(
            "-----BEGIN PGP SIGNATURE-----\nfake\n-----END PGP SIGNATURE-----\n"
        )
        self.pubkey = self.tmp / "pubkey.asc"
        self.pubkey.write_text(
    ...
    def _pipeline(self, *, dearmor_rc=0, gpgv_rc=0, validsig_fpr=None,
                  make_keyring=True):
        """subprocess.run side_effect for the (gpg --dearmor, gpgv) pair.

        dearmor: writes the keyring file (so the exists() guard passes) unless
        make_keyring is False; returns dearmor_rc. gpgv: returns gpgv_rc plus a
        --status-fd VALIDSIG line whose primary-key field is validsig_fpr
        (defaults to the pinned master fpr) when gpgv_rc == 0.
        """
        fpr = RELEASE_MASTER_FPR if validsig_fpr is None else validsig_fpr

        def side_effect(cmd, *a, **k):
            if "--dearmor" in cmd:
                if make_keyring:
                    Path(cmd[cmd.index("-o") + 1]).write_bytes(b"binary-keyring")
                return MagicMock(returncode=dearmor_rc, stdout="", stderr="")
            if cmd and cmd[0] == "gpgv":
                stdout = ""
                if gpgv_rc == 0:
                    # VALIDSIG <signing-fpr> <date> <ts> <exp> <ver> <res>
                    #          <pkalgo> <hashalgo> <class> <primary-fpr>
                    stdout = (
                        f"[GNUPG:] GOODSIG 0 InterGenOS\n"
                        f"[GNUPG:] VALIDSIG {fpr} 2026-01-01 0 0 4 0 1 8 00 {fpr}\n"
                    )
                return MagicMock(returncode=gpgv_rc, stdout=stdout, stderr="")
            return MagicMock(returncode=0, stdout="", stderr="")

        return side_effect

    @patch("installer.backend.integrity.subprocess.run")
    def test_valid_pinned_signature_returns_true(self, mock_run):
        mock_run.side_effect = self._pipeline()
        self.assertTrue(verify_manifest_signature(self.manifest, self.pubkey))
        cmds = [c.args[0] for c in mock_run.call_args_list]
        # (b): gpgv is the verifier, fed by a dearmor — and the abandoned
        # gpg `--keyring`/`--no-default-keyring` path must NOT be used.
        self.assertTrue(any(c and c[0] == "gpgv" for c in cmds))
        self.assertTrue(any("--dearmor" in c for c in cmds))
        for c in cmds:
            self.assertNotIn("--no-default-keyring", c)

    @patch("installer.backend.integrity.subprocess.run")
    def test_missing_sig_fails_closed(self, mock_run):
        # (a): a manifest with no detached .sig is not trustable and must not
        # even reach the verifier.
        self.sig.unlink()
        self.assertFalse(verify_manifest_signature(self.manifest, self.pubkey))
        mock_run.assert_not_called()

    @patch("installer.backend.integrity.subprocess.run")
    def test_foreign_key_returns_false(self, mock_run):
        # (c): the media-rewrite attacker — gpgv exits clean but the primary
        # fpr is NOT the pinned master. Must be rejected.
        mock_run.side_effect = self._pipeline(validsig_fpr="DEADBEEF" * 5)
        self.assertFalse(verify_manifest_signature(self.manifest, self.pubkey))

    @patch("installer.backend.integrity.subprocess.run")
    def test_good_exit_but_no_validsig_returns_false(self, mock_run):
        # gpgv rc=0 yet no VALIDSIG line (no parseable good sig) → reject.
        def side_effect(cmd, *a, **k):
            if "--dearmor" in cmd:
                Path(cmd[cmd.index("-o") + 1]).write_bytes(b"k")
                return MagicMock(returncode=0, stdout="", stderr="")
            return MagicMock(returncode=0, stdout="[GNUPG:] NEWSIG\n", stderr="")
        mock_run.side_effect = side_effect
        self.assertFalse(verify_manifest_signature(self.manifest, self.pubkey))

    @patch("installer.backend.integrity.subprocess.run")
    def test_bad_signature_returns_false(self, mock_run):
        mock_run.side_effect = self._pipeline(gpgv_rc=1)
        self.assertFalse(verify_manifest_signature(self.manifest, self.pubkey))

    @patch("installer.backend.integrity.subprocess.run")
    def test_dearmor_failure_returns_false(self, mock_run):
        mock_run.side_effect = self._pipeline(dearmor_rc=2, make_keyring=False)
        self.assertFalse(verify_manifest_signature(self.manifest, self.pubkey))
    ...
    def test_timeout_returns_false(self, mock_run):
        ...
        mock_run.side_effect = sp.TimeoutExpired(cmd=["gpgv"], timeout=30)
        self.assertFalse(verify_manifest_signature(self.manifest, self.pubkey))
