...
- **Test harness** — over 600 tests in `installer/tests/` covering installer backend, MOK validation, install-integrity, and Class 1 signing-chain verification; Phase A scaffold for GRUB `check_signatures=enforce` empirical validation. A further over 4,900 tests across the repo-level suites under `tests/` (preflight, repo-publish, SBOM, upstream-check, download-sources, and more).
...
R001.x is the first public release line; the project is in active development. The current release is always stated at [https://intergenos.org/news.html](https://intergenos.org/news.html). Originally built 2015-2016 (build_001 through build_003 on GitHub). Revived March 2026.
...
**Secure Boot validated on bare metal (first hardware, July 2026).** The full signed chain — Microsoft-signed shim → MOK-signed GRUB → MOK-signed Unified Kernel Image → `MODULE_SIG_FORCE=y` modules — runs end-to-end under enforced Secure Boot, with per-machine Machine Owner Key enrollment through MokManager, after earlier end-to-end validation in a virtual machine. Kernel post-install re-signs the per-machine UKI automatically, so every kernel update stays inside the enforced chain. Enforcement follows the firmware setting: with Secure Boot on and the per-machine key enrolled, the chain is enforced; with Secure Boot off, the same signed chain boots unenforced. A fresh install does not turn Secure Boot on for you.
...
**Mirror and signing.** The public binary mirror is live at `repo.intergenos.org`, serving package archives whose SHA-256 values are covered by a signed `InterGenOS.db` index; the archives are not individually signed. That is by design: the index signature covers every archive's SHA-256, `pkm` refuses any archive whose hash is not in the signed index, and the installation image carries its own signed archive manifest, so every archive is authenticated through one signature. The signing-key ceremony is complete (RSA-4096 master with hardware-token signing subkeys).

**Test harness.** over 600 tests in `installer/tests/` covering installer backend, MOK validation, install-integrity, and Class 1 signing-chain verification, plus over 4,900 tests across the repo-level suites under `tests/` (preflight, repo-publish, SBOM, upstream-check, download-sources, and more).
