...
#
# hipcub 7.2.4 — CUB-compatible interface over rocPRIM (header-only install)
# Source: projects/hipcub inside rocm-libraries
#
# Header-only: BUILD_TEST/BUILD_BENCHMARK/BUILD_EXAMPLE default OFF in
# the pinned CMakeLists (lines 65-73) and stay OFF. Resolves rocprim via
# the default PACKAGE fetch method against the installed rocprim package;
# the offline chroot makes any silent DOWNLOAD fallback fail loudly.
