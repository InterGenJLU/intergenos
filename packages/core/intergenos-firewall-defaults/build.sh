...
#
# Per D-011 (2026-05-19 requirement), this package is THE source
...
do_install() {
    ...
    # Ship an empty /etc/nftables.d/ so the conf's `include
    # "/etc/nftables.d/*.conf"` glob always resolves (an absent directory can
    # make nft error). Opt-in service fragments (Welcomer "Enable Services"
    # page) land here at runtime; shipping it empty keeps the default posture
    # default-deny — nothing is opened until a user toggles a service on.
    install -dm755 "${DESTDIR}/etc/nftables.d"
