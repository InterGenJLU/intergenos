...
# Forwarding is OFF by default, and it is a posture choice rather than a
# hardening reflex. With forwarding on, anyone who authenticates — or anyone
# holding a key that was taken from someone who did — can open arbitrary TCP
# connections THROUGH this machine to anything it can reach, and can reach back
# into the agent on the machine they came from. That is a network pivot and a
# credential reach that the person running the machine never asked for, granted
# silently by the act of allowing a login.
#
# Measured on an installed R001.2-03 machine 2026-09-16 (`sshd -T`):
# allowtcpforwarding yes, allowagentforwarding yes.
#
# A person who wants forwarding turns it on deliberately and knows they did:
# a file in /etc/ssh/sshd_config.d/ that sorts after this one (for example
# 90-local.conf) with `AllowTcpForwarding yes`, then restart the service. The
# user guide carries that sentence too.
