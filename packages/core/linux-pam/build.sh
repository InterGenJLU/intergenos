...
do_install() {
    ...
    # Entire configuration set ships as owned payload (hook-contract
    # wave: hooks may not write package-ownable bytes). Byte/mode-identical
    # to the files the retired post_install wrote on live targets.
    ...
    install -dm755 "${DESTDIR}"/etc/security
    install -dm755 "${DESTDIR}"/etc/security/limits.d

    # /etc/security/faillock.conf — pam_faillock rate-limiting on auth attempts.
    # Approved RHEL 9 baseline 2026-05-19: deny=5 (lock after 5 failures),
    ...
    cat > "${DESTDIR}"/etc/security/faillock.conf << "EOF"
# /etc/security/faillock.conf — pam_faillock configuration
# InterGenOS T0-4-B PAM-side closure (approved RHEL 9 baseline 2026-05-19).
...
EOF
    ...
    # Approved RHEL 9 baseline 2026-05-19: minlen=12 dcredit=-1
    # ucredit=-1 lcredit=-1 ocredit=-1 (require >=1 digit + >=1 upper + >=1
    # lower + >=1 other character class; min length 12).
    cat > "${DESTDIR}"/etc/security/pwquality.conf << "EOF"
...
EOF
    ...
    # Approved RHEL 9 + CIS-aligned baseline 2026-05-20:
    # - core 0 (suppress core dumps to prevent secret leakage; DISA STIG)
    # - maxlogins 50 (per-user concurrent login DoS mitigation; CIS 5.4.4;
    #   raised from 10 in GBC001.2 for crash-loop recovery headroom)
    # - nproc 65535 (per-user process count; root unlimited per RHEL convention)
    # - nofile 65535 (per-user file descriptor cap)
    cat > "${DESTDIR}"/etc/security/limits.d/00-intergenos.conf << "EOF"
# /etc/security/limits.d/00-intergenos.conf — pam_limits.so per-user caps
# InterGenOS T0-4-B PAM-side closure (approved RHEL 9 + CIS
...
# DoS mitigation; CIS 5.4.4 baseline. Raised 10 -> 50 (GBC001.2):
# 50 is still a hard ceiling no honest interactive user reaches, but
# gives recovery headroom so a crash-loop's orphaned VT/SSH sessions
# can't self-lock the box out of SSH (we hit exactly that at 10 during
# the GBC001.1 libEGL crash-loop). The root cause of that churn is fixed
# in libglvnd; this keeps the DoS cap while preventing lock-yourself-out.
*    hard    maxlogins    50
...
EOF
    ...
    # /etc/pam.d/other is OWNED BY shadow-pam (its variant is the live-
    # target bytes); the duplicate writer here was retired in the wave.

    # Live-target modes preserved (all 644)
    chmod 644 "${DESTDIR}/etc/security/faillock.conf"
    chmod 644 "${DESTDIR}/etc/security/pwquality.conf"
    chmod 644 "${DESTDIR}/etc/security/limits.d/00-intergenos.conf"
    chmod 644 "${DESTDIR}/etc/pam.d/system-account"
    chmod 644 "${DESTDIR}/etc/pam.d/system-auth"
    chmod 644 "${DESTDIR}/etc/pam.d/system-session"
    chmod 644 "${DESTDIR}/etc/pam.d/systemd-user"
    chmod 644 "${DESTDIR}/etc/pam.d/system-password"
}
