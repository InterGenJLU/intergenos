...
post_install() {
    set -e
    # memcached user/group is declared by
    # /usr/lib/sysusers.d/memcached.conf and created by the pkm canonical
    # sysusers hook before this lifecycle hook runs. A dedicated user
    # avoids running as `nobody` (universal account; lateral-movement
    # surface if compromised); the daemon is contained to its own state
    # directories.
