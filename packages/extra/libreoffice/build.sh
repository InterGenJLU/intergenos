...
do_install() {
    ...
    # distro-pack-install drops scp2 installer-module filelists (gid_Module_*)
    # at the DESTDIR root. They are packaging metadata, not payload — left in
    # place they enter the pkm manifest as phantom root-level files and
    # pkm verify reports them missing on every pristine install (35 entries,
    # 2026-07-20 pre-install eval). Same removal every mainstream distro does.
    rm -f "$DESTDIR"/gid_Module_*
}
