...
# self-signed cert with a real CA-signed one if needed. The self-signed
# cert is generated per-machine on the target at first service start
# (haproxy-tls-keygen.service), never at build time.
...
do_install() {
    ...
    install -d -m 755 "$DESTDIR/etc/haproxy/ssl"
    install -Dm755 "$BUILD_DIR/generate-tls-cert.sh" \
        "$DESTDIR/usr/lib/haproxy/generate-tls-cert.sh"
    install -Dm644 "$BUILD_DIR/haproxy-tls-keygen.service" \
        "$DESTDIR/usr/lib/systemd/system/haproxy-tls-keygen.service"
}

post_install() {
    set -e
    # haproxy user/group is declared by /usr/lib/sysusers.d/haproxy.conf
    ...
    # /etc/haproxy is NOT created here: do_install already stages
    # haproxy.cfg and etc/haproxy/ssl under it, so the directory ships as
    # owned payload and re-creating it from the hook would place a
    # manifest-invisible copy over one pkm owns (hook-contract wave).
    ...
    # The self-signed cert + key at /etc/haproxy/ssl/server.pem are NOT
    # generated here. haproxy-tls-keygen.service (which haproxy.service
    # Requires=) generates them per-machine on the target at first service
