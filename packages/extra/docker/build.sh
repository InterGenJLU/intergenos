...
do_install() {
    ...
    # moby man pages (dockerd.8 etc — generated by moby/man/Makefile if invoked,
    # but moby's man build is optional and our build() doesn't run it; ship
    # the source markdown manually-converted dockerd.8 if it lands. Skip the
    # moby/man Makefile invocation to avoid the extra build-time go-md2man
    # cycle — the engine binary's --help is the authoritative doc surface.)
    if [ -f "$MOBY_SRC/man/man8/dockerd.8" ]; then
        install -Dm644 "$MOBY_SRC/man/man8/dockerd.8" "$DESTDIR/usr/share/man/man8/dockerd.8"
    fi
