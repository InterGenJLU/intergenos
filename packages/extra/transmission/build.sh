...
#     the ExternalProject byproduct. Same user-control-aligned answer
...
configure() {
    ...
    # Out-of-tree CMake build. Release, NOT RelWithDebInfo (GE-01 L27):
    # the -g on this gtkmm-heavy C++ tree shipped 100-145MB per binary /
    # 745MB installed ("with debug_info, not stripped" — 49x oversized on
    # the signed manifest). The two workaround flags ride the exported
    # CXXFLAGS above, which cmake honors alongside Release's -O3 -DNDEBUG.
    cmake -S . -B build                                 \
