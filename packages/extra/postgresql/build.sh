...
post_install() {
    set -e
    # postgres user/group is declared by
    # /usr/lib/sysusers.d/postgresql.conf and created by the pkm
    # canonical sysusers hook before this lifecycle hook runs. The
    # daemon refuses to run as root; the user is mandatory.
