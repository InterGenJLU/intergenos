...
InterGenOS holds a security-only posture: security is not first, it is
only. That posture requires the system to ship strong cryptography
enabled by default (TLS, full-disk encryption, code signing, GPG,
Secure Boot, hardware-token integration), because user trust in the
...
**FIDO2 / WebAuthn.** InterGenOS support for FIDO2-based LUKS unlock
(experimental) is built on publicly available libfido2 / libcbor
stacks.
...
This notice was authored 2026-05-18 as part of InterGenOS v1.0
legal-readiness work. It establishes the project's U.S. BIS and EU
dual-use export-control posture for a system that ships strong
cryptography enabled by default.
