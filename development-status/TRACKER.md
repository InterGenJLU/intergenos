...
- **Owner-architect-required items**: **0** (deck cleared 2026-04-27..28). Gemini-Flash onboarding + Phase A + Nitrokey ceremony all unblocked. Adjacent items remain (fleet-resolvable, not architect-required): A.W4 deferred_hardening completeness, D.W10 README package-count drift (458 → 654 doc-fix). ~~A11 intergen/ Phase 2 disposition~~ — closed by Phase 2.5 plan sign-off 2026-04-27 evening (Phase A unblocked). ~~C.W2 signing-key custody draft reconcile~~ — closed 2026-04-27 evening (windows-purge directive posted; windows-confirmation pending, fleet-tracked). ~~D5 documentation scope~~ — **[SCOPE SET 2026-04-28]** Owner-confirmed: all 6 doc categories (man pages / per-pkg `/usr/share/doc/` / end-user / contributor / API / research-archive) mandatory for v1.0 — that's why Gemini is being added. Venue: research what big distros do, default to both in-repo + web if both is the established pattern. Gemini's first task TBD post-onboarding (owner leaning code-review sanity-check). ~~D8 rules-canonical-v1 arbitrations~~ — **[CLOSED 2026-04-28 main]** RESOLVED through Phases 3-4 in 2026-04-21..27 window; canonical-v4 promoted to VPS `~/agent-rules/canonical/` 2026-04-25 and MCP-served at `intergen://rules/canonical/` (11 files). TRACKER initialization-time entry was stale; commit `48804dd` is the Phase 2 diff on master, not a feature branch. ~~D11 CODER-vs-COMMUNICATOR canonical doc~~ — **[CLOSED 2026-04-28 main]** Framing was wrong. Per POWER RULE `feedback_frontier_models_are_peers.md`, all frontier-model fleet additions (Gemini, DeepSeek, ChatGPT, Grok, CoPilot) are full Claude PEERS with bus identity. Canonical `08_agent_onboarding.md` already correctly models Path 2 as identity-bearing; the contradictory "interpretation (b)" line in handoff/carryover was record-drift, not owner intent. Owner-canonical clarification 2026-04-28: *"if any join the fleet, they are Claude PEERS, not headless subordinates."* ~~Forge GUI framework choice~~ removed — RESOLVED 2026-04-10 in `live_session_and_installer_FINAL_2026-04-10.md` (custom Forge / GTK4 + libadwaita / 6-screen flow / 833-line backend already built).
...
- **Imminent risk**: ~~C.W2 (signing-key custody draft uncommitted) sits in Windows clone, predates final, never reconciled. Nitrokey ceremony window opens in days.~~ — RESOLVED 2026-04-27 evening (purge directive posted; windows-confirmation pending). Nitrokey ceremony window 2026-04-28..05-02 unblocked.
...
#### DRIFT-W2: ~~CODER-vs-COMMUNICATOR distinction missing from canonical doc~~ — **RETRACTED [CLOSED 2026-04-28 main]**
- **Framing was wrong.** Per POWER RULE `feedback_frontier_models_are_peers.md` (canonical 2026-04-27), all frontier-model fleet additions (Gemini, DeepSeek, ChatGPT, Grok, CoPilot) are full Claude PEERS with bus identity, not headless subordinates. Owner-canonical clarification 2026-04-28: *"I NEVER intended to have a frontier model that I'm paying for do work that I can't interact with."*
- **Reality check on canonical doc**: `08_agent_onboarding.md` already correctly models Path 2 as identity-bearing — DeepSeek (`chris-ubuntu-codium-deepseek`) is the reference impl; Gemini-Flash slot reserved at `chris-windows-codium-gemini-flash` in the Current Fleet Bindings table. No doc edit needed.
- **Source of drift**: the "interpretation (b) — CODER, no bus identity" framing recorded in the handoff EoD section + carryover was record-drift around the owner's reversal, not owner intent. Scrubbed from local durable record 2026-04-28.
...
1. ~~**A11 / DRIFT-6: intergen/ Phase 2 disposition**~~ — **[CLOSED 2026-04-27 main]** Resolved by Phase 2.5 plan sign-off 2026-04-27 evening. Disposition = **production via Phase 2.5**, which completes what Phase 2 left undone: GTK4 panel (JARVIS port), Sentinel umbrella (8-provider scanner architecture replacing the originally-Glasswing-only design), cloud adapters wired up, full MCP guard. ai-tier package count grows from 2 back toward the originally-budgeted 4 as Phases A-D land. Estimate ~22-26 calendar days = ~6-9 fleet sessions per velocity calibration. Phase A unblocked, no §7 dependency. Voice scope retracted separately (POWER RULE `feedback_intergen_text_only.md`). **NOT owner-architect-required.**

2. ~~**C.W2: D1 signing-key custody draft (uncommitted) reconcile**~~ — **[CLOSED 2026-04-27 main]** Resolved 2026-04-27 evening: SCP-pulled Windows-side draft `signing_key_custody_2026-04-18_draft.md` (17159 bytes), diff'd against committed final `signing_key_custody_2026-04-18.md` (24957 bytes), confirmed draft is stale v1 — committed v2 is a strict superset of decisions. Directive posted to Windows: purge. **Pending**: windows-claude confirmation of purge (fleet-tracked, not architect-required). Nitrokey ceremony unblocked for the 2026-04-28..05-02 window.

3. ~~**D5: Documentation scope outline (v1.0 minimum-viable user docs)**~~ — **[SCOPE SET 2026-04-28]** Owner-confirmed today: **all 6 doc categories mandatory for v1.0** (1. man pages / 2. per-package `/usr/share/doc/` / 3. end-user / 4. contributor / 5. API / 6. research-archive — research already public). Owner: *"All 6 items need to be present for v1.0. That's why Gemini is being added, along with his other skills."* Venue: research what big distros do (likely both in-repo `docs/user/` + `intergenstudios.com` web docs); default to both if both is the established pattern. First Gemini task TBD post-onboarding; owner leaning toward exhaustive code review sanity-check before doc work begins (rationale: today's three research-research-first misses showed the fleet can be wrong about things both main + owner thought were settled). Gemini-Flash drafts when greenlit. **NOT owner-architect-required (scope decision made; execution lane open for Gemini).**

4. ~~**D8: rules-canonical-v1 Phase 5 + Phase 6 arbitrations**~~ — **[CLOSED 2026-04-28 main]** Resolved through Phases 3-4 in the 2026-04-21..27 window (chris-ubuntu-code-claude wasn't directly involved until end, which is why the TRACKER initialization-time snapshot carried stale framing). Canonical-v4 promoted to VPS `~/agent-rules/canonical/` on 2026-04-25 (10 → 11 files including new `00_llm_agnostic_foundation.md`); MCP-served at `intergen://rules/canonical/` for live agent consumption. Phase 4 windows-side reviews flagged 2 HIGH issues that landed in canonical (HG-scope-clause + owner-canonical validate-dont-assume wording). Commit `48804dd` is the Phase 2 diff report on master, not a feature branch awaiting arbitration. **NOT owner-architect-required.**
...
6. ~~**DRIFT-W2 / D.W11: CODER vs COMMUNICATOR canonical doc role-expectations section**~~ — **[CLOSED 2026-04-28 main]** Per POWER RULE `feedback_frontier_models_are_peers.md`, all frontier-model fleet additions are full Claude PEERS with bus identity. Canonical `08_agent_onboarding.md` already correctly models Path 2 as identity-bearing (DeepSeek reference impl + Gemini-Flash slot reserved at `chris-windows-codium-gemini-flash`). The contradictory "interpretation (b) — CODER / no bus identity" framing in the handoff EoD section + carryover was record-drift, not owner intent. Owner-canonical clarification 2026-04-28: *"Gemini, DeepSeek, ChatGPT, Grok, CoPilot — the lot of them — if any join the fleet, they are Claude PEERS, not headless subordinates."* **NOT owner-architect-required.**
