...
| BOOT | Measured-boot scope (PCR / TPM-sealing) | (a) implement `systemd-cryptenroll` LUKS+TPM2 + PCR-policy OR (b) drop "measured boot" claim from docs | PROPOSED | — | (pending — decision #2) | audit B-006; remediation plan decision #2 | — |
...
| PARTITION | Manual / advanced partitioning | DEFERRED to v1.x (LVM, LUKS, manual layout) | DEFERRED | 2026-04-10 | operator (design) | FINAL design doc | — |
| PARTITION | LUKS / encryption-at-rest for v1.0 | NO encryption-at-rest in v1.0; zero references in installer code | UNKNOWN | — | (no explicit decision) | audit Lane N | only LUKS reference anywhere is signing-key custody (USB backup, orthogonal) |
...
| SECURITY | TPM-sealed MOK posture | MOK keys TPM-sealed where feasible; current key bit-length needs uplift to 3072/4096 | PROPOSED | 2026-05-18 | (pending — F-013) | remediation plan #2 | — |
...
4. Measured-boot scope (implement or drop from docs — couples to B-006 + B-050 + LUKS decision).
