...
Peer-reviews delivered (APPROVE in all cases): `ed4cfa2a` keyring (with defensive-assert suggestion absorbed), `f2b3cf87` H-009, `9d85139e` H-005, `8b10ad46` H-011/H-021. Plus pre-approve on windows-host coordinator's H-006/H-018 exit-code scheme proposal. No installer-side regressions surfaced.
...
| **SC4** CLI + DB integrity | ~12 H-rows + O-001 reinstall + H-007/H-008 helpers | **4 rows landed** (H-005, H-009, H-011/H-021, H-013); ~8 remaining incl. H-006/H-018 (pre-approved scheme; windows-host coordinator engaging), H-004, H-007, H-008, H-022, H-023, H-024, O-001 |
...
| **SC6** DR + transparency + GPL source | L-022 + L-023 + L-024 + P-001 (3 items) | **All 3 P-001 items + L-022 + L-023 landed** (5 of 6); L-024 transparency log publish-side remaining |
...
- SC4 remaining 8 rows (windows-host coordinator + installed-system coordinator) — likely landable
- L-024 transparency log publish-side (build-system coordinator) — likely landable
- SC3 tier 4 start (windows-host coordinator) — partial likely
...
**My honest projection at session-end:** T0-5 will be approximately 60-80% closed at morning. No "complete" claim per D-009 item 7. The remaining 20-40% is mostly SC5 upgrade-safety which has the highest design-cost per row.
...
2. **Decide on L-024 transparency log** — the publish-side wiring needs a transparency-log target (sigstore/Rekor? self-hosted cosigner? defer to v1.1?). This is a real architectural call. The audit row is in the queue but the design decision feeds in.
