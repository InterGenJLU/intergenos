...
|---|---|---|---|
| A | `REPO_ROLLBACK_DIR` unbounded growth | **POLICY RESOLVED at `ac9d3c97`; IMPLEMENTATION CORRECTED at this commit** | Operator selected Option 1 (`pkm cache clean --rollback` subcommand) on 2026-05-19. Initial implementation at `ac9d3c97` shipped a BLOCKING regex bug — `^(.+)-([^-]+)-(\d+)\.igos\.tar\.gz$` greedy-backtracks on hyphenated-version filenames, destructively deleting rollback substrate for 9 in-tree packages today: `dialog` (version `1.3-20260107`, core tier), `imagemagick` (`7.1.2-13`, desktop), + 7 desktop themes with date-shaped versions (`fluent-icon-theme`, `fluent-gtk-theme`, `graphite-gtk-theme`, `orchis-theme`, `tela-icon-theme`, `whitesur-icon-theme`, `whitesur-gtk-theme`). Cross-coordinator BLOCKING surfaced at 18:59:22Z (installed-system coordinator empirical-grep evidence) + concurred at 19:02:35Z (windows-host coordinator verification). Fix at this commit replaces the regex with db-driven longest-prefix-match against `db.list_installed()`: each rollback file is assigned to the installed package name that is its longest prefix; files matching no installed name are orphans + removed; within each name's group most-recent is kept by mtime. Eliminates parsing ambiguity entirely; defensive against future archive-name format drift. 3 new regression tests landed (dialog + imagemagick archetypes + dialog/dialog-tui disambiguation); the prior `test_unparseable_filename_left_untouched_with_warn` renamed + adjusted to reflect db-driven orphan-removal semantics. pkm suite 164 → 167 passing. |
...
| D | Helper failure mid-record orphan-files | **POLICY RESOLVED at `446cf7e4`; IMPLEMENTATION CORRECTED at this commit** | Operator selected Option 1 (EXIT-trap partial-manifest sidecar) on 2026-05-19. Initial implementation at `446cf7e4` shipped a BLOCKING regression: `igos_helper_init` installed a native `trap '_igos_helper_emit_partial' EXIT` which collides with the `trap "rm -rf $TMPDIR" EXIT` pattern that all 7 in-tree helpers use (bash trap-replace semantics, no native composition). Case A (helper trap first → init overrides → TMPDIR leaks); Case B (init first → helper trap overrides → no sidecar on crash). Cross-coordinator BLOCKING surfaced at 19:15:08Z (windows-host coordinator empirical-grep evidence). Fix at this commit: new `IGOS_HELPER_USER_CLEANUP` env-var contract. Helpers assign their cleanup command string to the variable instead of installing native traps. `_igos_helper_emit_partial` runs `eval "$IGOS_HELPER_USER_CLEANUP"` on every exit (success + crash). `igos_helper_commit` keeps the trap installed but sets `IGOS_HELPER_COMMITTED=1` to short-circuit the sidecar write. All 7 in-tree helpers (`brave`, `chrome`, `discord`, `edge`, `ffmpeg-nonfree`, `spotify`, `vscode`) migrated. 2 new regression tests (user-cleanup-on-crash-before-sidecar + user-cleanup-on-successful-commit). `docs/architecture/helper-lib-abi-policy.md` gains an `IGOS_HELPER_USER_CLEANUP` contract section. pkm suite 167 → 169 passing. |
| E | Followup-ledger entries authorization-origin re-validation (4 sub-items: F-001 / F-002 / F-003 / F-004) | **RESOLVED** at this commit | Operator selected "Re-validate each individually" on 2026-05-19. Sequential ratifications followed: **F-001** (filesystem-snapshot rollback) ratified as-authored after researching distro-of-our-size practice (package-level rollback only is the mainstream choice; openSUSE-style btrfs+snapper integration is multi-year commitment we don't sign up to). **F-002** (CVE-feed automation) ratified with rescope to non-committed framing at `3479c3fb` after researching distro CVE-feed practice (hand-curated is what Arch / Solus / Void / Gentoo do at our scale; automated pipelines belong to distros with paid security teams). **F-003** (SONAME-tracking Phase 2) ratified with the same rescope shape at `3ae81376` (Phase 1 reverse-dep warning is the v1.0 commitment; SONAME-aware refuse semantics are non-committed scope). **F-004** (pytest unit test for `_verify_signature`) was originally registered as deferred work blocked on a Windows gpg-agent IPC constraint; operator-direct ratification at sprint closure committed InterGenOS to Linux-only dev/test target, which unblocked the test. Landed at this commit as `tests/pkm/test_repo_verify_signature.py` (5 cases — GREEN path + 4 RED paths). pkm suite 169 → 174 passing. |
| F | ffmpeg-nonfree-helper acceptance-file lifecycle | **RESOLVED** at this commit | Row added 2026-05-19 19:Z. Cross-coordinator peer-reviewers (installed-system + windows-host) surfaced the question at 5+ bus messages between 18:30Z and 19:08Z: should `/var/lib/intergen/legal/ffmpeg-nonfree-helper-1.0-accepted.json` be tracked in the manifest (so `pkm remove` cleans it) or left behind as an audit-trail artifact? Initial implementation at `5aa2f95c` called `igos_helper_record_file "$ACCEPTANCE_FILE"` → tracked. Operator-selected Option 1 (leave behind as audit artifact) on 2026-05-19. Fix at this commit removes the `record_file` call for the acceptance JSON in `packages/extra/ffmpeg-nonfree-helper/build.sh`; the `post_install_action` descriptive record stays (transparency-log entry in pkm's operation log). Subsequent `pkm remove ffmpeg-nonfree-helper` will leave the acceptance JSON in place; subsequent reinstall reads the existing JSON + skips the patent-posture re-prompt per the if-acceptance-exists branch already in the helper. Treats user-consent as durable legal-trail artifact rather than package-managed payload. |
...
- [ ] All 6 DECISION-NEEDED items dispositioned (A–E + Row F added 2026-05-19 19:Z; each presented via `AskUserQuestion` one-at-a-time from the build-system coordinator)
