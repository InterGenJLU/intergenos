...
- We **do** ship opt-in helper packages for users who explicitly
  ...
  applicable to them (`ffmpeg-nonfree`).
...
**Distribution practice:** Fedora and Debian explicitly omit
FDK-AAC from their main repositories. RPM Fusion / Debian non-free
are the typical opt-in channels.
...
- Users who specifically want FDK-AAC quality install the
  **`ffmpeg-nonfree`** package via `pkm install
  ffmpeg-nonfree`. The helper:
...
where `ffmpeg-nonfree` has been used implicates whatever
...
as part of `packages/desktop/ffmpeg`'s libavcodec, together with
the two encoder libraries the same ffmpeg build links —
`packages/desktop/x264` (H.264) and `packages/desktop/x265`
(H.265).
...
**Distribution practice:** Fedora ships software decoders for
H.264/H.265 in modern releases, having determined that defensive
omissions did not meaningfully reduce patent exposure for ordinary
use. Debian ships in `main` (decoders) and `non-free-firmware`
(some encoder acceleration).

**InterGenOS disposition:** **Shipped in default — decoding and
encoding both.** Decided 2026-08-05: the default ffmpeg build is
configured with `--enable-libx264` and `--enable-libx265`
(`packages/desktop/ffmpeg/build.sh`), so it links the H.264 and
H.265 encoders, and both encoder libraries
(`packages/desktop/x264`, `packages/desktop/x265`) are
`tier: desktop` with no `iso_include` override — which the recipe
parser resolves to inclusion — so they are on the default
installation medium. The same build also compiles in ffmpeg's
NVENC and NVDEC entry points (`--enable-ffnvcodec`,
`--enable-nvenc`, `--enable-nvdec`), which are hardware encode and
decode paths rather than additional software codecs: no NVIDIA
code is linked or redistributed, the paths load the driver's own
libraries at run time and are simply unavailable where those
libraries are absent, and the driver package that supplies them
(`packages/extra/nvidia`) is `tier: extra` and mirror-only, so it
reaches a system only as a separate user install. The
encoding-side patent posture is the user's and the
redistributor's to evaluate, as it is for the video transcoder in
§ 4.9: encoding at commercial scale, or distributing an encoder as
part of a commercial product, is the fact pattern the licensing
pools address, while ordinary personal encoding from camera
...
**Distribution practice:** Universally omitted from main
repositories. VideoLAN operates a separate distribution for their
`libdvdcss` package that ships under defensive jurisdiction.
...
### 4.9 H.264 / H.265 encoding in the video transcoder (HandBrake)

**Component:** `packages/extra/handbrake` — HandBrakeCLI plus the `ghb`
GTK front-end. `tier: extra` with `iso_include: false`, so it is
**mirror-only**: it is not on the default installation medium and
reaches a system only when the user installs it.

**What it builds and links.** HandBrake's Linux build compiles seven
source tarballs of its own, staged offline and sha256-checked twice
(once by the builder's pin, once by HandBrake's own recorded hash).
Two of them carry codec patent exposure: **x265** (the HEVC encoder,
compiled additionally at 8/10/12-bit depth) and the patched **ffmpeg**
it requires. The H.264 encoder **x264** is not a HandBrake contrib on
Linux — it comes from this tree (`packages/desktop/x264`), as do
theora, vorbis, ogg, lame, opus, speex and vpx.

**Patent exposure:** H.264 (AVC) and H.265 (HEVC) **encoding** is the
side of these codecs that patent pools (Via-LA, HEVC Advance, Access
Advance) license most actively, and encoder licensing terms differ
from decoder terms in most jurisdictions. Encoding at commercial
scale, or distributing an encoder as part of a commercial product, is
the fact pattern those pools address.

**Distribution practice:** Fedora ships HandBrake through RPM Fusion
rather than the main repository; Debian ships `handbrake` in `main`
with the same x264/x265 linkage.

**InterGenOS disposition:** **Not on the default medium; available
...
keeps the package clear of vendor SDK terms. A user who installs
HandBrake is choosing to run an encoder; §6 applies to anyone
redistributing a system with it installed.

### 4.10 Theora (libtheora)

**Component:** `packages/extra/libtheora` — the Xiph.Org Theora video
codec, BSD-3-Clause. `tier: extra` with `iso_include: false`, so it is
**mirror-only**; its only consumer in this tree is HandBrake (§4.9),
which links it from the system rather than building its own copy.

**Patent exposure:** Theora derives from On2's VP3, whose patents On2
irrevocably licensed royalty-free for any purpose before donating the
codec to Xiph.Org; the format was designed and released on that
footing. It is not administered by any of the codec licensing pools
named in §1.

**InterGenOS disposition:** **Not on the default medium**, for a
packaging reason rather than a patent one — nothing on the default
medium consumes it. It is listed here so the register covers every
codec package the tree gained, including the ones whose posture is
unremarkable.
...
2. **Decide whether to include the opt-in patent-encumbered
   helpers** (`ffmpeg-nonfree` etc.). The default ISO
...
- [`docs/governance/license-policy.md`](../governance/license-policy.md)
  — the project's license-discipline policy, including handling of
