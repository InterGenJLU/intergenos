...
specifications the campaign builds on. **NOT committed; the coordinator reviews and lands. Only the
operator declares resolved.**
**Owner:** InterGenJLU · **Maintained by:** the coordinator · **Author:** the icon-design workstream. **Companion to:**
...
- **Stroke = ~2% of canvas detailed / ~6% simplified** → **5px / 16px** on 256, hybrid
  ...
  from-anchor. Supersedes the VL §11 anatomy-table 12px/24px, which the coordinator reconciled to §6
  on the public tree.]
...
Proposed here (operator ratifies), grounded in the `palette.yaml` accent set. Every value is
...
(the operator's taste loop, at emblem level). Shipped: the eight folder-class emblems
...
symbolic color *names* (so it survives recoloring), not baked hex. [Flagged for the coordinator/operator,
...
- The operator's taste loop stays at **template + batch level**: author a base shape / emblem,
