...
**Declared GPU targets.** The build chroot has no GPU, so target-sensitive packages carry an explicit `gpu_targets` declaration in `package.yml` — the build compiles for the declared architecture list, never auto-detects. One field carries both vendors' ISA spellings: AMD `gfx` identifiers and NVIDIA compute capabilities in the `CMAKE_CUDA_ARCHITECTURES` vocabulary (`75-virtual;86-real;120a-real`). The parser validates each token against a closed grammar, so a malformed target list dies at parse time rather than reaching a compiler.

Twenty-two recipes — the whole ROCm stack, from `rocblas` and `hipblaslt` up to `llama-cpp-hip` — declare the SAME AMD list, because the engine's matrix multiplies resolve into those libraries and an architecture declared in one place and missing from another fails on the user's machine at model load rather than in the build. The list is written out in each recipe, since a recipe field has no cross-recipe constant to point at, and `tests/igos_build/test_the_rocm_stack_declares_one_target_list.py` keeps the copies byte-identical. `installer/backend/gpu_detect.py` holds a further copy, because the installer runs where the mirror-only HIP package is not present; its own parity test holds it equal to the engine recipe.
