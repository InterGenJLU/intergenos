...
**Status:** In effect for the v1 library API.

This policy governs the `intergenos-helper-lib` shell API and its
companion manifest schema. For the manifest schema itself, see
...
- **Breaking changes** ship as a new major version (`helper-lib-v2`),
  ...
  bump plus a `Supersedes: intergenos-helper-lib < 2.0` entry, using
  pkm's SUPERSEDES primitive.
...
1. `packages/core/intergenos-helper-lib/package.yml` bumps `version`
   ...
   package, as specified by pkm's SUPERSEDES primitive.
...
- **By review:** the v1 `helper-lib.sh` comments reference this policy
  document, and reviewers reject breaking changes that do not also ship
  a major-version bump and the accompanying SUPERSEDES wiring.
- **Mechanically (planned for a v1.x release):** a pre-commit gate that
  asserts the `IGOS_HELPER_LIB_API_VERSION` constant matches the
  declared major version and that each v1 API function is still present
  in the library. This is a planned hardening item, not part of the
  initial v1.0 surface.

## References

- [`docs/architecture/helper-manifest-spec-v1.md`](helper-manifest-spec-v1.md) — manifest schema specification
...
- [`pkm/installer.py`](../../pkm/installer.py) — the reader path (`HELPER_MANIFEST_DIR` and `_run_helper`)
- [`docs/components/pkm.md`](../components/pkm.md) — pkm, including the SUPERSEDES primitive
