...
**Status:** v1.0 — minimum-scope implementation complete. The D-008 amendment pulled spotlighting of retrieved content and per-conversation trust state from the §10 v1.x set into v1.0 minimum scope; both are now implemented. The §14.3 audit-log retention default is a 30-day logrotate policy plus an `intergen tool-log --clear` CLI. This design is the structural resolution path for audit findings I-027 (`SafetyTier.CONFIRM` not enforced) and I-035 (`manage_services` LLM-root-equivalence). It composes with D-007 (root and SSH posture).

**Scope at v1.0:**

- **v1.0 minimum** — required for ship. Implemented before the first ISO that includes InterGen. Includes the D-008 amendment expansion (spotlighting and per-conversation trust state).
- **v1.x full** — tracked on the v1.x backlog. Narrowed by amendment to cover the §5.2 advisory-to-gating elevation, cross-conversation policy, and output-side scanning.
...
The action is a reasonable next step the user would expect but did not literally name. Examples:
...
Implemented in the v1.0 minimum surface:
...
- §5.2 explicit instruction-pattern detection — **advisory** in v1.0 per §5.2; scanner at `intergen/pattern_detect.py`; injection corpus at `tests/intergen/injection_corpus/`
...
- §7 review modal (zenity-based subprocess modal at `intergen/review_modal.py` — sidesteps GTK main-thread + event-loop constraints since the dispatcher can be invoked from any thread) + §7.2 notification fallback (`notify-send` + 1-hour implicit-Deny)
...
- §9 audit log (`intergen/audit_log.py` — `XDG_STATE_HOME`-resolved JSONL writer with `0o600` file permissions, 30-day logrotate retention per §14.3, and an `intergen tool-log --clear` user-wipe CLI in `intergen/cli.py`)
- **Spotlighting of retrieved content** (per D-008 amendment) — `intergen/spotlighting.py` wraps every ingress-tool result in `<UNTRUSTED-INGRESS source="...">...</UNTRUSTED-INGRESS>` markers at tool-result construction, with spoof-marker escaping via `_SPOOF_GUARD_PATTERN`
- **Per-conversation trust state** (per D-008 amendment) — `ConversationTrustState` in `intergen/interfaces/provenance.py` records symmetric allow/deny decisions keyed by `(tool_name, source_attribution)`; the router resets it on conversation end via `reset_conversation_state()`

Composes with D-007 Option A's pkexec gate (tracked as separate v1.0 work; the pkexec gate integrates against the stable `verify_tool_call()` / `ToolRegistry.execute()` / `IngressTracker` / `ConversationTrustState` surface).

### v1.x full (backlog)

Adds (narrowed by the D-008 amendment; spotlighting and per-conversation trust state already landed in v1.0):

- §5.2 explicit-pattern detection **elevated from advisory to gating** (after the false-positive rate is calibrated against real telemetry; this depends on the v1.0 advisory mode shipping first to gather that data)
...
LoC totals reflect the v1.0 minimum authoring surface.

| Surface | LoC actual | Sanity-test surface |
|---|---|---|
| `ToolCall.source_of_request` required field + `__post_init__` validator (`intergen/interfaces/types.py`) | +22 | ToolCall construction rejects missing label |
| Provenance taxonomy + `IngressTracker` + `ConversationTrustState` + `DispatchDecision` + `AuditRecord` + `escalate_provenance` (`intergen/interfaces/provenance.py` NEW) | 254 | 11 gate-path sanity checks |
| Audit log writer + reader + clear + 0o600 perms + XDG path resolution (`intergen/audit_log.py` NEW) | 140 | 7 audit-log sanity checks (write/read/clear cycle + malformed-line resilience + file mode) |
| Spotlighting wrapper + spoof-marker escape + extract regions (`intergen/spotlighting.py` NEW; per D-008 amendment) | 139 | spoof-marker escape verified against adversarial close-marker |
| Dispatcher gate `verify_tool_call` + the §6 behavior matrix + `record_user_decision` + `build_audit_record` (`intergen/provenance.py` NEW) | 303 | 9 dispatcher gate sanity checks (matrix + escalation + symmetric trust state + missing-source reject + user-decision recording + audit construction) |
| Registry gate integration + `_PRIVILEGED_TOOLS` + `_classify_risk_tier` + audit-log-on-every-dispatch + I-027 closure (`intergen/tool_registry.py`) | +291 | imports + tier classification + unknown-tool reject |
| Router instance state (`_ingress_tracker` + `_trust_state`) + per-turn reset + `reset_conversation_state()` + P3 LLM-tools gate wiring + P1/P2 keyword-match `USER_DIRECT` ToolCall construction (`intergen/router.py`) | +~80 | 8 router-wiring sanity checks |
| System-prompt `_PROVENANCE_DIRECTIVE` + `ToolSchema.to_openai()` source_of_request injection + `_extract_provenance` helper + both `stream_with_tools` yield sites updated (`intergen/llm.py` + `intergen/interfaces/types.py`) | +~95 | 8 system-prompt sanity checks (toOpenAI inject + extract pops/maps + benign/invalid/missing handling) |
| Review modal — zenity primary + notify-send fallback + 1-hour implicit-Deny + 2-arg callback factory (`intergen/review_modal.py` NEW) | 293 | 7 modal sanity checks (format + truncation + session-detection + callback shape) |
| `intergen tool-log` CLI subcommand — read/--clear/--json/--count/--limit (`intergen/cli.py`) | +~95 | 9 CLI sanity checks (human render + JSONL + count + limit + clear + empty + main dispatch) |
| Pattern detection scanner — corpus-agnostic `scan_for_injection_patterns()` (`intergen/pattern_detect.py` NEW) | 96 | scanner sanity checks (empty + multi-match + malformed-regex skip + truncation) |
| Injection corpus integration tests — parametrized pytest consuming the corpus at `tests/intergen/injection_corpus/` (`tests/intergen/test_injection_corpus.py` NEW) | 211 (tests) + corpus TBD | 3 pass + 3 skip-on-empty-corpus; tests light up as corpus entries are populated |
| Integration with D-007 pkexec gate (privileged-tier `hold_for_review` path is the integration point) | (separate; engaged against the stable interface) | end-to-end test on a built ISO |
| Documentation (user-facing security-defaults section + developer guide) | small | review |

**Total v1.0: ~1,942 lines across InterGen** (304 modified plus 1,638 new across 7 new modules and 1 new test file), in addition to this design document, the audit-finding closures, and roughly 100-200 lines in the Forge installer for pkexec wiring (tracked as separate installer work).
...
**Resolution:** NOT in ingress set. Rationale: false-positive rate on legitimate status queries swamps the gate without commensurate security gain (service-name strings carrying injection bytes is a theoretical attack but the watermark in §5.1 already covers the broader case).

### 14.2 — Notification fallback timeout — RESOLVED

Question: timeout for held actions when session is locked?

**Resolution:** One-hour fixed expiry with implicit Deny. Configurability is v1.x backlog if telemetry shows the fixed default is wrong; v1.0 ships the fixed value to gather data.

### 14.3 — Audit log rotation policy + user-data-deletion path — RESOLVED

Question: retention period + user-wipe path?

**Resolution:** A 30-day logrotate policy via a `logrotate.d` snippet shipped by the intergen package, plus an `intergen tool-log --clear` CLI for user-initiated wipe at any time. This mirrors the Debian/Ubuntu sudo/auth/journal retention model: a sensible system-set bound that the user can always override, keeping the user in control of their own data.

### 14.4 — Cross-host coordination — v1.x scope (§10)

Question: if a user runs InterGen on multiple InterGenOS machines, does the per-conversation trust state sync?

**Status:** Explicit v1.x scope per §10. The likelihood of multi-host InterGen use is currently low but becomes plausible once sync infrastructure exists; this is a v1.x design item to revisit at that point.

### 14.5 — Pattern detection corpus ownership — RESOLVED

Question: where does the v1.0 advisory pattern set and the v1.x gating pattern set live?

**Resolution:** The injection corpus lives at `tests/intergen/injection_corpus/`, seeded with baseline entries drawn from published threat reports. The scanner module (`intergen/pattern_detect.py`) and the test infrastructure (`tests/intergen/test_injection_corpus.py`) consume it. v1.x calibration uses telemetry from real-world use to elevate §5.2 from advisory to gating.
...
This design is approved and implemented at v1.0 minimum scope. Review covered the dispatcher gate, the tool-call schema design, the system-prompt extension, the cross-host coordination question, the user-facing documentation surface, and integration with pkexec and the ISO compliance gate.
