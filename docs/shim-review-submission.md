...
- **Kernel-config fragment terms:** `00-universal-baseline.config` is the cross-distro convergence baseline (Ubuntu/Arch/Fedora/Debian/openSUSE intersection of recommended hardening), applied first. `99-intergenos-overrides.config` is applied second and takes precedence — concatenation order in `packages/core/linux-kernel/build.sh:36` followed by `make olddefconfig`. Citations below name the fragment file and the config symbol itself rather than a line number: symbol names are unique within a fragment, so `grep -n '^CONFIG_SECURITY_LOCKDOWN_LSM_EARLY=' config/kernel/fragments/99-intergenos-overrides.config` locates any cited setting exactly, and the citation does not go stale when the fragment is regenerated. The final compiled kernel `.config` is the merged result.
...
**Audit-gap discovery + resolution (Q17):** during template population, this submission surfaced an InterGenOS kernel-config gap — baseline set `CONFIG_LOCK_DOWN_KERNEL_FORCE_NONE=y` without `CONFIG_LOCK_DOWN_IN_EFI_SECURE_BOOT=y` that Ubuntu/OpenSUSE/Fedora all ship, leaving lockdown=integrity not auto-triggered under Secure Boot. The first resolution (a one-line override addition) merged to master at commit `baf84d8`; it was superseded on 2026-06-02 by `CONFIG_LOCK_DOWN_KERNEL_FORCE_INTEGRITY=y`, which engages lockdown unconditionally and is what the tree ships today. See the Q17 body for the enforcement description and the full audit trail.
...
- **Compiler / linker default:** GCC (≥ 8) and binutils (≥ 2.41, the version pinned in InterGenOS toolchain) default to `-z noexecstack` in their link-line behaviour. The own-shim build recipe `docker/shim-build/Dockerfile` does not override this default with `-z execstack` (the link line at `Dockerfile:147-149` invokes `make -j1 VENDOR_CERT_FILE=… DEFAULT_LOADER=…` without `LDFLAGS` overrides). The `packages/core/shim-signed/` path is the Fedora-piggyback recipe, distinct from this own-shim build.
...
- **Hardware enforcement:** `CONFIG_X86_64=y` (`config/kernel/fragments/99-intergenos-overrides.config`) implies x86_64 NX-bit hardware support is in use. The CPU enforces NX on any segment marked non-executable at the page level.
...
- own-shim build recipe: `docker/shim-build/Dockerfile` (source-of-truth for the binary submitted to rhboot/shim-review)
- Fedora-piggyback shim recipe (separate path, not the own-shim submission): `packages/core/shim-signed/build.sh`
- Toolchain: the shim is **not** built with the InterGenOS toolchain. It is built inside the pinned container defined by `docker/shim-build/Dockerfile` — a digest-pinned `debian:bookworm-slim` base whose apt packages are further pinned to a `snapshot.debian.org` timestamp, so the compiler and binutils versions are reproducible and fixed by that snapshot rather than floating. The Dockerfile is the authority for the exact versions; quoting them here would rot independently of the build that produces the submitted binary.
...
- `CONFIG_SECURITY_LOCKDOWN_LSM=y` — set in `config/kernel/fragments/00-universal-baseline.config`
- `CONFIG_SECURITY_LOCKDOWN_LSM_EARLY=y` — set in `config/kernel/fragments/99-intergenos-overrides.config`, under its `--- Security ---` section (ensures lockdown LSM is registered before any module-load decisions)
- `CONFIG_MODULE_SIG=y` and `CONFIG_MODULE_SIG_ALL=y` — set in `00-universal-baseline.config`; `CONFIG_MODULE_SIG_FORCE=y` — set in `99-intergenos-overrides.config`, under its `--- Forge Secure Boot (kernel signing) ---` section. Together these are the module-signing enforcement chain.
- `CONFIG_INTEGRITY=y`, `CONFIG_INTEGRITY_ASYMMETRIC_KEYS=y`, `CONFIG_INTEGRITY_TRUSTED_KEYRING=y`, `CONFIG_INTEGRITY_PLATFORM_KEYRING=y`, `CONFIG_INTEGRITY_MACHINE_KEYRING=y` — full integrity subsystem, all set in `00-universal-baseline.config` (the `CONFIG_INTEGRITY*` symbols are contiguous there)
...
InterGenOS ships `CONFIG_LOCK_DOWN_KERNEL_FORCE_INTEGRITY=y` in `config/kernel/fragments/99-intergenos-overrides.config`. The kernel boots in `lockdown=integrity` mode **unconditionally** — engaged from early boot on every boot, regardless of the Secure Boot state — so no runtime Secure-Boot detection, manual boot-parameter, or `/sys/kernel/security/lockdown` write is required. The lockdown LSM is registered early via `CONFIG_SECURITY_LOCKDOWN_LSM_EARLY=y` (override), so lockdown is in force before any module-load decisions.

We deliberately do **not** use `CONFIG_LOCK_DOWN_IN_EFI_SECURE_BOOT` (auto-engage lockdown only when EFI Secure Boot is detected at runtime): that option is a downstream distro patch (Fedora/SUSE/Debian carry it) and is **not** present in vanilla kernel.org mainline — verified absent from the checksum-matched 6.18.10 source. InterGenOS builds the vanilla kernel and ships a fully signed boot chain with a sealed UKI cmdline — Secure Boot enforcement itself is optional and ships off by default on current target hardware — so unconditional `FORCE_INTEGRITY`, which engages regardless of the Secure Boot state, is strictly stronger and more deterministic than runtime-Secure-Boot-detection coupling, and stays patch-free. (Migrated from the `IN_EFI_SECURE_BOOT` approach to the vanilla-native `FORCE_INTEGRITY` mechanism 2026-06-02; the full rationale is the comment block immediately above `CONFIG_LOCK_DOWN_KERNEL_FORCE_INTEGRITY=y` in `99-intergenos-overrides.config`.)

### What lockdown=integrity enforces

When in integrity mode, the LSM enforces the standard upstream restrictions: blocks `/dev/mem` / `/dev/kmem` / `/dev/port` write, restricts `kexec_load`, disables `/proc/kcore`, restricts BPF privileged operations, restricts DMA from untrusted devices, hard-rejects unsigned kernel modules (via `CONFIG_MODULE_SIG_FORCE=y` in the override fragment), restricts kprobes / mmiotrace / tracing-of-kernel-data.

### Distro alignment

Fedora and OpenSUSE reach lockdown by runtime promotion: they set `CONFIG_LOCK_DOWN_IN_EFI_SECURE_BOOT=y` with `FORCE_NONE=y` as the default level, so the kernel is unlocked unless firmware reports Secure Boot on. Verified from `docs/research/kernel_configs/{fedora,opensuse}.config`. Ubuntu uses an older Kconfig name (`LOCK_DOWN_IN_SECURE_BOOT`) with equivalent semantics. InterGenOS does not follow that pattern: it sets `CONFIG_LOCK_DOWN_KERNEL_FORCE_INTEGRITY=y` and leaves `CONFIG_LOCK_DOWN_KERNEL_FORCE_NONE` unset in the override fragment, so lockdown is engaged on every boot rather than promoted on detection. `CONFIG_LOCK_DOWN_IN_EFI_SECURE_BOOT` is not set anywhere in the InterGenOS fragments, because the option does not exist in the vanilla kernel this tree builds.
...
**Yes — four upstream-CVE backport patches**, declared in `packages/core/linux-kernel/package.yml` and applied — each patch's `sha256sum` checked against its declaration first — by the loop at `packages/core/linux-kernel/build.sh:112-136` before configure. All four target known CVE classes in 6.18.10 + were backported from upstream stable / authoritative-source references between 2026-03 and 2026-05. Reviewer-runnable verification: `ls build/patches/` plus inspection of the `patches:` declaration and the patch-application loop.

| Patch file | CVE | Upstream source | Class |
|---|---|---|---|
| `CVE-2026-31431-copy-fail.patch` | CVE-2026-31431 | Herbert Xu, 2026-03-26 — `crypto: algif_aead - Revert to operating out-of-place` | Use-after-free in AEAD in-place optimization; root-from-unprivileged-local |
| `CVE-2026-43284-xfrm-esp-shared-frags.patch` | CVE-2026-43284 | Kuan-Ting Chen, upstream stable backport | xfrm/ESP shared-fragments handling defect |
| `CVE-2026-43500-rxrpc-shared-frags.patch` | CVE-2026-43500 | InterGenJLU custom backport of rxrpc paged-frag fix to 6.18.10 | rxrpc shared-fragment handling defect |
| `CVE-2026-46300-fragnesia-skbuff-coalesce.patch` | CVE-2026-46300 | William Bowling, upstream — "Fragnesia" skbuff coalesce | Skbuff fragment coalesce defect |

Patches landed in two commits: `fa3feb9b` (CVE-2026-31431 originally) + `ed09604d` (Dirty Frag / Fragnesia kernel security sprint adding the three CVE-2026-43284 + CVE-2026-43500 + CVE-2026-46300 backports).
...
Per `docs/ephemeral-module-signing.md` (referenced from the `--- Forge Secure Boot (kernel signing) ---` section of `99-intergenos-overrides.config`): the InterGenOS kernel build auto-generates an ephemeral RSA keypair per build (the kernel Makefile creates `certs/signing_key.pem` because `CONFIG_MODULE_SIG_KEY` is intentionally unset), signs all in-tree modules with that key during the build, embeds the corresponding public-key certificate in the built `vmlinuz` image, and **the keypair file lives only inside the per-build sandbox** — pkm cleans the entire build directory at end-of-package-build, so the private key has no persistence past the build artifact production. There is no long-lived signing-key file anywhere on disk; consequently there is no key file to leak.
...
- **The ephemeral key is reaped at end of kernel build.** The kernel Makefile generates the keypair into the build sandbox during `make modules`, signs modules, embeds the pubkey in `vmlinuz`. pkm then packages the kernel binary (NOT the certs directory) and discards the entire build sandbox per its normal end-of-package-build cleanup. The ephemeral keypair file therefore has no on-disk persistence past the build sandbox lifetime.
...
- **Module signature validation:** the kernel image's built-in keyring contains the matching pubkey. `CONFIG_MODULE_SIG_FORCE=y` (`99-intergenos-overrides.config`) ensures unsigned-module load is HARD-rejected (not warn-and-load).
- **The InterGenOS Unified Kernel Image (UKI) is signed by the InterGenOS vendor cert** — the same X.509 cert embedded in shim's `vendor_cert` slot. The UKI is assembled by `ukify build` (the systemd-canonical wrapper around `systemd-stub`; see `scripts/build-uki.sh`) into a single PE binary that bundles `vmlinuz` + `initramfs` + kernel `cmdline` + the `linux` SBAT entry into one signed envelope. `sbsign` then applies the vendor-cert signature to that single envelope at release time per `docs/signing-procedure.md`. (`ukify` replaces a prior raw `objcopy --add-section` assembly path that had a PE `SizeOfImage` defect causing strict-UEFI rejection; switching to `ukify` is the canonical systemd-supported wrapper and the current source-of-truth.) This anchors the trust chain end-to-end:
...
- **`CONFIG_MODULE_SIG_KEY` is intentionally unset** (`99-intergenos-overrides.config`, where the line reads `# CONFIG_MODULE_SIG_KEY is not set`). This forces the kernel build process to auto-generate the ephemeral keypair on every build, rather than reusing a long-lived signing key.
...
- `config/kernel/fragments/99-intergenos-overrides.config`, the `--- Forge Secure Boot (kernel signing) ---` section (the kernel-config encoding)
...
**Cross-host reproducibility evidence (native-Linux Docker; both attested against current RSA-4096 vendor cert):**

| Host | Distro | Docker | Tarball SHA-256 | Shim SHA-256 |
|---|---|---|---|---|
| Build host A (build VM, native `docker.io`) | Ubuntu 24.04.2 LTS | `docker.io 27.5.x` (apt-installed) | `32776256647f3c28d8ac800b299471d5eb593448e4a75097e802ccad17ab5e25` | `441f9bd1bb75d5dbfc9c5d2c8451b210c9156573515923786d0a1cc4a2a01e25` |
| Build host C (independent witness, native `docker.io`) | Ubuntu 24.04 LTS (`ubuntu-hplt`) | `docker.io 29.4.3` (apt-installed) | `32776256647f3c28d8ac800b299471d5eb593448e4a75097e802ccad17ab5e25` | `441f9bd1bb75d5dbfc9c5d2c8451b210c9156573515923786d0a1cc4a2a01e25` |

Both hosts produce byte-identical SHAs despite different physical hardware, different docker-engine major versions (27.x on Host A vs 29.x on Host C), and different apt-snapshot policies — confirming the Dockerfile's `apt-snapshot` pin to `20260501T000000Z bookworm`, `SOURCE_DATE_EPOCH=1746489600`, `make -j1`, and content-addressed base image (`debian:bookworm-slim@sha256:5a2a80d11944804c01b8619bc967e31801ec39bf3257ab80b91070eb23625644`) effectively remove host-environment as a leak source. Docker-major-version diversity (27 vs 29) is the substantive cross-host attestation property; same-Ubuntu-major-version is incidental (Host C was the readily-available reproducibility witness).

The Host C attestation log (9-check `verify-b2-reproducibility.sh` format) is staged at `~/intergenos/shim-review-evidence/host-c-verify-20260521.log` for inclusion in the shim-review fork `logs/` directory at PR-open time. An earlier "Build host B (Ubuntu 22.04)" framing in pre-cert-regen drafts referenced an aspirational second witness that did not materialize against the current cert; the Host C attestation supersedes that.
...
**No EV certificate is *embedded* in the shim** — the certificate embedded in the shim is the InterGenOS vendor CA (see Q28), not an EV cert.

**Important (corrected 2026-06-11):** an EV (Extended Validation) code-signing certificate **is** separately required to sign the `.cab` file uploaded to the Microsoft Hardware Dev Center File Signing Services — a distinct certificate from the one embedded in the shim. This requirement applies to InterGenOS's submission; the EV cert's issuer/subject is disclosed in the dedicated shim-review `README` field. An earlier draft incorrectly claimed our path avoids the EV cert because "Red Hat batches submissions to Microsoft" — that is not how shim-review works: shim-review provides the community review (and waives only the annual third-party security audit), while the distro itself holds the Microsoft Hardware Dev Center account, signs the `.cab` with its own EV certificate, and uploads it.
...
- **Lifetime:** 2 years (notAfter 2028-05-13 per cert ASN.1) with documented rotation strategy per Q21
...
| Crypto + signature verification | `pgp`, `gcry_sha256`, `gcry_sha512`, `gcry_rsa` | `pgp` + `gcry_*` provide the verification primitives. The shim-lock verifier that enforces the shim → GRUB → UKI signature chain under Secure Boot is BUILT-IN to GRUB 2.14's EFI kernel (`kern/efi/sb.c`) and is not a loadable module — it self-registers when GRUB starts on a system with shim. Listing `shim_lock` in `--modules=` would fail `grub-mkstandalone` with "cannot open shim_lock.mod" (see `scripts/build-grub-standalone.sh:96-100`). |
...
| Display | `gfxterm`, `gfxmenu`, `videoinfo`, `efi_gop`, `font`, `png` | Boot-time display + theme rendering for the 3-entry menu. `vbe` (VESA BIOS Extensions) is BIOS-only and does not exist in the x86_64-EFI module tree — EFI uses `efi_gop` (Graphics Output Protocol) for the same role (see `scripts/build-grub-standalone.sh:107-111`). |
| Script pattern-matching | `regexp` | The menu configuration extracts the `WxH` prefix from `$gfxmode`, which GRUB reports as either `WxH` or `WxHxC` depending on the firmware/driver path; without `regexp` the resolution-specific background lookup fails, because the shipped images are named `WxH` only. Matches the installed-system mechanism in `/etc/grub.d/06_intergenos_multi_background`, which depends on `regexp` through `grub-mkconfig_lib`. |
| Modules the generated installed-system config loads | `efi_uga`, `gettext`, `gzio`, `bli` | Baked into the signed image rather than left as loose files on the ESP. Under Secure Boot the built-in shim_lock verifier refuses any module loaded as a loose ESP file (`prohibited by secure boot policy`, `kern/efi/sb.c`), and each of these names, while missing, printed that refusal on the boot console at every menu display. `efi_uga` is the `00_header` `load_video` fallback (read-only display protocol); `gettext` is `00_header` menu translation (read-only); `gzio` reads a gzip-compressed kernel or initramfs and is required for the fallback and recovery entries to load; `bli` sets the Boot Loader Interface EFI variables that `25_bli` uses — the same spec-defined identification variables the UKI stub sets on the primary path, which is a different thing from the `loadenv`/`savedefault` write-environment-to-disk class excluded below. |
...
(verified against `packages/core/grub/package.yml`)
  ...
  GNU GRUB2 version 2.14, sourced from upstream (`https://ftpmirror.gnu.org/gnu/grub/grub-2.14.tar.xz`).
...
The InterGenOS-signed Linux kernel enforces signed-only kernel-module loading via `CONFIG_MODULE_SIG_FORCE=y` (override fragment). The lockdown LSM enforces additional restrictions on every boot, independently of Secure Boot state, because the kernel is built with `CONFIG_LOCK_DOWN_KERNEL_FORCE_INTEGRITY=y`; see Q17 for the enforcement description.
...
- `CONFIG_SECURITY_LOCKDOWN_LSM=y` (baseline fragment) + `CONFIG_SECURITY_LOCKDOWN_LSM_EARLY=y` (override fragment) — see Q16, Q17
- `CONFIG_MODULE_SIG=y` + `CONFIG_MODULE_SIG_ALL=y` (baseline fragment) + `CONFIG_MODULE_SIG_FORCE=y` (override fragment) — signed-only modules, hard-reject
- `CONFIG_MODULE_SIG_KEY` intentionally unset (override fragment) — ephemeral per-build key, see Q19
- Full `CONFIG_INTEGRITY_*` chain (baseline fragment) — IMA + integrity machine keyring + integrity platform keyring
- `CONFIG_X86_64=y` (override fragment) — 64-bit only, NX-bit hardware enforcement implicit
- `CONFIG_DEBUG_WX=y` — kernel debug-time check rejects W+X mappings (verified at boot, panics on violation)

**Patches:** see Q18.

**Lockdown:** engaged unconditionally, not auto-triggered. `CONFIG_LOCK_DOWN_KERNEL_FORCE_INTEGRITY=y` is set in `99-intergenos-overrides.config` and `CONFIG_LOCK_DOWN_IN_EFI_SECURE_BOOT` is not set — see Q17 for the full enforcement description and audit trail.
...
The kernel-lockdown gap (Q17) was surfaced during this draft's population pass and resolved ahead of submission — first at master commit `baf84d8`, then by the `CONFIG_LOCK_DOWN_KERNEL_FORCE_INTEGRITY=y` mechanism that supersedes it and ships today. Documented here for transparency.
...
## Draft completion checklist (for peer-review)
