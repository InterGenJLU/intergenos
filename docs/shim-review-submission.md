...
| Display | `gfxterm`, `gfxmenu`, `videoinfo`, `efi_gop`, `font`, `png` | Boot-time display + theme rendering for the 3-entry menu. `vbe` (VESA BIOS Extensions) is BIOS-only and does not exist in the x86_64-EFI module tree — EFI uses `efi_gop` (Graphics Output Protocol) for the same role (see `scripts/build-grub-standalone.sh:84-86`). |
