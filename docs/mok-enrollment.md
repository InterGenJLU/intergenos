...
1. **Generate keypair** ([mok.py:generate_mok_keypair](../installer/backend/mok.py#L54)):
   ...
   - **What the machine trusts is stated at every login.** The Welcomer's first page says how many certificates carrying this project's machine owner name the firmware trusts, whether this machine's own is among them by fingerprint, which of the others were kept on purpose and which nobody has decided about, and the `mokutil --export` / `mokutil --delete` pair that retires the rest. It runs as the person: it reads `mokutil`'s listings, the public certificate copy at `/etc/intergenos/mok.der`, and `/etc/intergenos/mok-prior-keys` — the world-readable record the install writes, one fingerprint per line, `kept=` or `retired=`, which is the only way that page can tell a key somebody chose to keep from a key nobody was ever offered. A count it could not read is shown as unreadable rather than as zero.
