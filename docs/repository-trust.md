...
1. **SHA-256 Verification:** pkm fetches the canonical repository index (InterGenOS.db) from the configured mirror — by default `https://repo.intergenos.org/x86_64/current/`, the path the published channel symlink resolves to. This index contains the expected SHA-256 hashes of all valid packages. Every downloaded .igos.tar.gz package archive is hashed locally. If the hash does not match the index perfectly, the installation is hard-rejected.
...
*   --archive-trust=repo-only: Also rejects the installation unless the archive hash matches the signed repository index. It differs from `strict` in intent rather than in that check — it states explicitly that the index is the only trust source being consulted for this archive.
...
InterGenOS builds from source and uses a vendor-tarball pipeline for dependency reproducibility, particularly for ecosystem-heavy tools like Rust/Cargo packages. The source URLs and canonical hashes (including vendor archives) are published in each package's manifest, ensuring the supply chain is auditable and repeatable.

## 4. Key Rotation

The subkeys used to sign packages operate on a 2-year cadence. When a key is rotated, the new keys will be distributed via a intergenos-keyring package update during a 30-day overlap window. The rollover announcements are signed by the offline master key, and the procedure is detailed in the [Signing Key Ceremony Procedure](ceremony/signing-key-ceremony-procedure.md). The maintainer-side multi-key trust window (a `docs/signing-key.asc` export carrying both subkeys, the committed binary keyring regenerated from it with its SHA-256 pin updated in the same commit, and additive `pkm/release-keys.json` entries) is documented in [docs/signing-key.md §Multi-Key Trust Window](signing-key.md#multi-key-trust-window-maintainer-procedure).
