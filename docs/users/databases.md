...
InterGenOS targets 14 curated databases across relational SQL, NoSQL document, Key-Value (KV) cache, embedded KV, time-series, distributed KV, and wide-column categories. See the [Database Landing Plan](../architecture/database-landing-plan.md) for the full v1.0 set; the packages landed on the mirror as of any given build are listed by `pkm search` against the live index.
...
| **Document storage with rich queries** | ferretdb | mongodb (Opt-in, SSPL licensed) |
| **In-memory cache** | valkey | memcached, redis (Opt-in, RSAL/SSPL licensed) |
...
### Document NoSQL (ferretdb, mongodb, couchdb)
...
### In-Memory Cache (valkey, redis, memcached)
...
### Specialized Workloads (influxdb3, etcd, cassandra)
