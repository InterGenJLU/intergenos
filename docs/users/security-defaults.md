...
- **No Unattended Signing**: the machine owner key that signs your kernels lives at `/var/lib/intergen/mok/mok.key`, readable only by the administrator and encrypted with a passphrase you set during the install. Nothing on this machine signs a boot image or a driver module without asking you for it, which also means a program running as root cannot. You are asked at a kernel or graphics-driver update; if the passphrase is not given, nothing is signed, the previous signed kernel stays the boot target, and the package manager says the new one is not bootable until signed. Machines installed before September 2026 hold a key with no passphrase and are offered one at their next update. The reasoning, the cost and what someone with your disk can do are in [Secure Boot and MOK](secure-boot-and-mok.md).
- **Weekly Discard Follows Your Disk Layout**: Once a week, an unencrypted install tells the drive which blocks its filesystem is no longer using (systemd's `fstrim.timer`), so a solid-state drive stops rewriting blocks it could have reused. An encrypted install does not, and does not merely leave the timer switched off: the encrypted root is opened without permission to pass discards down to the hardware at all. That is the point of it. Which blocks a filesystem is using is a description of that filesystem — roughly how full it is and where its data sits — and a drive that is told can be read later by someone who has the disk but not your passphrase. Because the permission is withheld at the mapping rather than at the timer, switching the timer on by itself changes nothing on an encrypted machine: what you would have to change is how the volume is opened, which trades that description of your filesystem for the drive's write performance and endurance. That is a real trade and yours to make; it is off here because an encrypted disk exists to keep exactly this from the hardware.
