...
- **Secure Boot**: Mandatory. The boot chain is anchored by a Microsoft-signed shim (we piggyback on Fedora's pre-signed shim) that validates our InterGenOS GRUB bootloader, which subsequently verifies our Linux kernel and Unified Kernel Images. We do not trust unsigned kernel modules. If you need out-of-tree modules like proprietary drivers, the Forge installer will walk you through enrolling a Machine Owner Key (MOK) on your first boot. Installed systems regenerate and sign each kernel's UKI with your machine's local MOK at every kernel install or upgrade, so the same boot-time signature verification continues to apply to kernels you install after the original ISO image. The InterGenOS release-signing key never leaves our HQ hardware token; only the MOK that Forge generates on your own machine signs the kernels you install. See the [Secure Boot and MOK Guide](secure-boot-and-mok.md) for a deeper walkthrough.
