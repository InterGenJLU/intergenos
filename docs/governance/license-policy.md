# InterGenOS — License Policy

**Audience:** project contributors, package maintainers, auditors, and
downstream redistributors who need to know what the project will and
will not ship.
...
for that). It is the engineering-discipline policy: which licenses we
accept, which we reject, what compound expressions look like, how we
handle borderline cases, and what auditors should check.
...
manager, the Forge installer, the InterGen assistant, the scripts,
the package templates, and the audit and verification
...
- `FDK-AAC` license — **rejected** as a shipped-binary default
  ...
  **only** to support an opt-in `ffmpeg-nonfree` flow
...
Visual Studio Code, Spotify, Discord, Brave, and the Anthropic
Claude Code CLI). These packages exist because users frequently want
them, and because the alternative — silently leaving users to install
proprietary software via vendor scripts that bypass all of our
...
with a canonical URL, a last-checked date, and the relevant clause
that the user is accepting.
...
|---|---|---|
| fdk-aac | `packages/desktop/fdk-aac/` | Allowed only as a source dep for opt-in ffmpeg-nonfree; never linked into default ffmpeg |
| ffmpeg | `packages/desktop/ffmpeg/` | Default build **drops** `--enable-nonfree` and `--enable-libfdk-aac`; opt-in nonfree variant ships as `ffmpeg-nonfree` |
...
|---|---|
| `packages/extra/chrome` | `LicenseRef-Google-Chrome-ToS` |
| `packages/extra/edge` | `LicenseRef-Microsoft-Edge-EULA` |
| `packages/extra/vscode` | `LicenseRef-Microsoft-VSCode-Software-License` |
| `packages/extra/spotify` | `LicenseRef-Spotify-ToS` |
| `packages/extra/discord` | `LicenseRef-Discord-ToS` |
| `packages/extra/brave` | `LicenseRef-Brave-EULA` |
| `packages/extra/claude-code` | `LicenseRef-Anthropic-Commercial-Terms` |
...
|---|---|---|---|
| InternVL3.5-2B GGUF (Tier 1) | Hugging Face `bartowski/OpenGVLab_InternVL3_5-2B-GGUF` | `Apache-2.0` | Required at first launch of InterGen (audit P-016) |
| Qwen3.5 GGUF (9B / 35B-A3B, Tiers 2–3) | Hugging Face `unsloth/Qwen3.5-*-GGUF` | `Apache-2.0` | Not required — permissive licenses are auto-accepted (see the note below) |
| nomic-embed-text-v1.5 | Hugging Face `nomic-ai/nomic-embed-text-v1.5-GGUF` | Apache-2.0 | Same flow |

> **Authority for a model's license is the signed manifest, not this table.**
> `intergen/data/models-manifest.json` carries a `license_ref` per model and
> ships with a detached signature verified against the release master key;
> `intergen/model_manager.py` reads the acceptance gate **only** from that
> signed field, and falls back to requiring acceptance for anything it does
> not recognise. Decided 2026-07-02: the Qwen3.5 entries declare `Apache-2.0`
> in the signed manifest, which un-gates their download. Any change to a
> model's license is made in the signed manifest first and reflected here
> second — if the two ever disagree, the manifest is correct and this row is
> the defect.
...
|---|---|---|
| `shim-x64.efi` (Fedora-signed for Microsoft UEFI CA) | `packages/core/shim-signed/` | Repackaged from the Fedora RPM. Redistribution permission documented at [`docs/governance/fedora-shim-redistribution.md`](fedora-shim-redistribution.md). To be replaced by an InterGenOS-owned, Microsoft-signed shim once shim-review completes. |
...
paths carry the header. Audit finding P-007 (which recorded that
none of the InterGenOS-authored source files carried headers at the
time of the audit) is closed: a bulk header-addition pass applied
the header across the authored tree.
...
A pre-push gate (`scripts/check-license-policy.py`, planned) will
automate the mechanical parts of the checklist.
...
- [`docs/governance/fedora-shim-redistribution.md`](fedora-shim-redistribution.md)
  — Fedora-signed shim redistribution permission (P-009).
...
legal-readiness review, closing audit finding **P-005** (High: no
license-compatibility audit for AGPL-3.0 packages; this document
also closes the broader gap of having no dedicated license-policy
document) from the 2026-05-18 comprehensive state audit.
...
maintainer and approval by the project owner. Clarifications,
