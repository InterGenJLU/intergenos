...
InterGenOS is an AI-integrated Linux distribution built entirely from source. It emphasizes a strict "user-control posture" and supply-chain defense-in-depth. Every component is designed to ensure the user retains ultimate authority over their machine and data.
...
InterGenOS organizes its 1,100+ packages into a strict tier system. This hierarchy dictates the build order and ensures a clean separation of concerns from the foundational compiler tools up to the user-facing desktop applications.
...
6.  **Compute (`packages/compute/`)**: The GPU compute platform — the ROCm stack and its SDKs. Built before the AI tier because the AI tier's GPU-native builds consume these at build time. Defined entirely as mirror-only; nothing in this tier ships on the installation image.
7.  **AI (`packages/ai/`)**: The specialized runtime components for the InterGen assistant, including `llama.cpp` and the `intergen` daemon itself. Two packages ship on the image; the remainder of the model-tooling stack is mirror-only.

A tier's default decides whether its packages ship on the image or are mirror-only, and
individual packages override that default in either direction — so the tier is the rule,
...
*   **Hardware-Tier Model Catalog** (canonical catalog in `intergen/model_manager.py`): the system provisions a model from the detected graphics hardware. **Tier selection keys on discrete-GPU presence and VRAM only — system RAM is never an input**, and unreadable VRAM fails *down* to Tier 1 rather than up. Every tier is vision-capable, each with a paired projector pinned alongside its model in the signed manifest.
    *   Tier 1 (InternVL3.5-2B): no discrete GPU. The CPU-friendly floor, by construction rather than as a fallback.
    *   Tier 2 (Qwen3.5-9B): a discrete GPU with roughly 7 GB of VRAM or more.
    *   Tier 3 (Qwen3.5-35B-A3B MoE): a discrete GPU with roughly 22 GB of VRAM or more.
