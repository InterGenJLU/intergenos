...
**Status:** Mitigated by in-tree kernel patches plus a defense-in-depth module blacklist. The mitigation was carried into the first public release: the R001 source tree declares all three patches for both kernel build passes, including the pass that produces the kernel an installed system boots. **Update 2026-09-11:** the kernel moved from 6.18.10 to 6.18.51, whose source carries all three fixes from the stable branch; the three backport patches were retired from the recipes (see "Kernel version for this line" below). The module blacklist remains.
...
**Kernel version for this line (revised 2026-09-11).** This advisory originally stated that 6.18.10 was the kernel of the R001 release line, that the pin was binding for it, and that backporting the fixes rather than moving the kernel version was the chosen path. That statement held for R001, R001.1 and R001.2, which shipped 6.18.10 with the three backports above. It is withdrawn as of 2026-09-11: the kernel moves to 6.18.51 (the current release of the 6.18 long-term series) for the next point release. The reason is a later flaw the backport approach did not cover — CVE-2026-53362, a local flaw in the IPv6 send path fixed upstream in 6.18.38 and listed in the CISA Known Exploited Vulnerabilities catalogue on 2026-08-27 — together with the roughly two thousand other fixes between 6.18.10 and 6.18.51, and the fact that the stable branch carries upstream's own reviewed fix for CVE-2026-43500 (commit `3eae0f4f` and its follow-up `46cb765e`, which decrypts from a copied buffer) in place of the locally authored backport. Before the move, the presence of each fix in the pristine 6.18.51 source was confirmed: the xfrm and skbuff backports reverse-apply cleanly there; the locally authored rxrpc backport does not (it targets code the later upstream refactor replaced) and was confirmed instead by the presence of upstream's replacing commits `3eae0f4f` and `46cb765e` in the source. The patches are then no longer declared, and the fixes reach the built kernel from the source tarball itself. The advisory for the newer flaw is `CVE-2026-53362-ipv6-fraggap.md` beside this file.
...
The decision to ship the blacklist by default reflects InterGenOS's user-control vision — a system you understand, can modify, and can trust — together with its security-only posture: security is not first, it is only.

- User control is preserved. The file is plainly visible and user-editable. Any user who needs IPsec ESP or OpenAFS can remove the file or run `modprobe esp4` manually.
- Modern consumer VPNs (WireGuard, OpenVPN) do not use the `esp4`/`esp6` modules. `rxrpc` is OpenAFS-only and extremely niche on desktop Linux. Blacklisting these by default breaks essentially no consumer-facing workflow on v1.0.
- The trade-off weighs convenience for the small number of users who depend on in-tree IPsec ESP against security for everyone else. Under a security-only posture, security wins.
