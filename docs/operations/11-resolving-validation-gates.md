...
| 7 | **Lifecycle-hook contract** (hooks are maintenance-only) | `check-hook-contract.py` | yes (exit 1 names each offending recipe, lifecycle event, line number and line; exit 2 on bad arguments or a recipe that cannot be read) |
| 8 | ISO-closure (effective-`iso_include` runtime edges) | `preflight-iso-closure.py` | yes |
| 9 | Scan A — build-order | `preflight-build-order.py` | yes |
| 10 | **`direct_install` lane contract** | `preflight-direct-install-lane.py` | yes (exit 1 on violations, exit 2 on usage/setup errors) |
| 11 | **Kernel-release lockstep** | `preflight-kernel-release-lockstep.py` | yes (exit 1 on violations, exit 2 on usage/setup errors) |
| 12 | License identifiers (SPDX list membership) | `preflight-license-identifiers.py` | yes (exit 2 — never a quiet pass — when the bundled identifier file is missing/malformed, when the package tree is empty, or when the selected scope leaves nothing to check) |
| 13 | Scan B — silent feature loss | `preflight-silent-loss.py` | yes (self-skips to exit 0 when no prior chroot — first build / post-revert; under `--require-audit` a skip AND any coverage gap — empty inventory, missing/unreadable log, missing package.yml — fail closed, exit 3) |
| 14 | **Bash-tier currency** — resume only | `preflight-bash-tier-currency.py` | yes on a `--start-at` resume with a populated chroot; self-skips otherwise. Exit 1 refuses a resume whose start phase would skip the rebuild of a stale or never-built bash-tier package, naming each package, its building phase, and the earliest `--start-at` that covers them all |
| 15 | Scan A.2 — undeclared build deps | `preflight-undeclared-deps.py` | yes (first run extracts source tarballs into `build/scan-cache/`, ~12 min; cached ~10s after; gate mode fails closed, exit 3, when a DECLARED source cannot be located/extracted/scanned or the sources dir is absent — `--advisory` for exploratory runs; unknown `--only` names are exit 2. `source: []` and non-archive payloads are named by-design classes, never failures) |
| 16 | Staged-kernel exclusivity | `preflight-single-kernel.sh` | yes (`--allow-none` here: passes on an absent/empty chroot; more than one staged kernel fails in every mode) |
...
Gate 7 (lifecycle-hook contract) asserts that a recipe's lifecycle functions stay maintenance-only. Those functions travel inside the signed archive and run on the target at install time, so anything they write is content the manifest does not declare, the archive's signature does not cover and `pkm verify` cannot check. A hook may enable a unit, refresh a cache or database, generate machine-unique state, or restore ownership and permissions on payload the archive already delivered. It may not write payload: an install, copy, move or link into `/usr`, `/opt` or `/boot`; a redirect or `tee` into those paths; the same forms creating a file or directory under `/etc`; or an in-place `sed`/`perl` edit of a shipped path. The scan reads the same function text the archive seal extracts, with comments stripped, and it tracks heredoc terminators so a config file's contents are never read as commands. All six lifecycle events are scanned, not just `post_install`. Resolve by moving the line into `do_install`, where the builder stages the file, the manifest records it and the archive carries it signed; weakening the gate is never the remedy. The gate is a regression guard over the shapes recipes actually use, not a proof of absence — a write through a variable path or a helper script is invisible to static analysis, and the package manager's ownership recorder is what observes those.
...
Gate 10 (`direct_install` lane contract) refuses the build when a recipe built by a bash driver declares `direct_install: true`. That flag is a contract with the builder: the recipe's `do_install` writes to absolute paths on the live filesystem, and the builder determines what the package owns by diffing filesystem snapshots taken around the build. Exactly one builder implements the other half — the Python builder takes the snapshots. The bash drivers never read the flag: they stage into DESTDIR, tar that, and succeed. The result is a package present on the live image and absent from every installed system, because the archive every install is built from holds only whatever was incidentally staged. Two recipes were measured in that state on an installed system, so the failure is a property of the lane rather than a mistake in one recipe. Resolve by moving the recipe to the builder that implements the contract, or by rewriting `do_install` to stage into DESTDIR like the rest of the bash lane.

Gate 11 (kernel-release lockstep) refuses the build when a recipe states a kernel-release path that the build will not produce. The kernel's release string is computed, not chosen: the kernel recipe's `release:` field is stamped into `CONFIG_LOCALVERSION` as `-igos-<release>`, and the resulting `<version>-igos-<release>` names the image in `/boot`, the module tree under `/usr/lib/modules`, the unified kernel image and the boot-menu entry. Recipes also write those paths by hand in `verify_paths:`, and a hand-written literal has no derivation behind it — it drifts whenever the release moves, which happened three times before this gate existed and was caught hours into a build by the squashfs path audit. The gate derives the expected release from the same field the kernel's `build.sh` reads and names any recipe stating a different one. Resolve by correcting the literal in the recipe, in the same change that bumps the kernel; the gate reports and refuses rather than rewriting, because a release-coupled path is a claim about what the build produces and correcting it silently would hide the drift.

Gate 12 (license identifiers) asserts that every recipe's `license:` is a real SPDX license expression whose identifiers are on the SPDX license list. That field is not decorative: the ISO SBOM publishes it as `licenseDeclared` and the mirror index carries it, so a declaration like `Public-Domain`, `MIT-style` or `Various (redistributable)` propagates into both and resolves for no SPDX consumer. Checking the *shape* of an expression is not enough — each of those is a well-formed token — so this gate checks list membership, and checks the right operand of `WITH` against the separate license-**exception** list (which is why `MIT WITH MIT` fails). Resolve it in the recipe, three ways and no others: use the SPDX identifier for the license the package actually carries; or, when SPDX has no identifier for that license, declare `LicenseRef-<Name>`, which is SPDX's own mechanism for exactly that case and is how `linux-firmware`'s mixed vendor blobs and the public-domain packages are declared; or, if the identifier is genuinely newer than the bundled list, refresh `config/spdx-license-list.json` per the `_regenerate` note inside that file. There is no acknowledge/override path, and `EXEMPT_PACKAGES` in the gate is empty by design — `LicenseRef-` already covers everything SPDX does not carry. **Deprecated-but-listed identifiers (`GPL-2.0`, `LGPL-2.1`) PASS and are printed as warnings**: replacing one with `-only` or `-or-later` resolves an ambiguity that only the package's own license text can settle, so the gate reports them and leaves the determination to a human. The identifier sets are bundled rather than fetched, because a gate that reaches the network makes its verdict depend on a third party being reachable and honest at that moment; the run prints the license-list version and release date every time, so a stale list is stated rather than assumed.

Gate 16 (staged-kernel exclusivity) asserts the chroot holds exactly one staged kernel — one `/usr/lib/modules/<kver>` tree and one `/boot/vmlinuz-<kver>`, mutually agreeing. The kernel's module dir and image are *release*-named (`CONFIG_LOCALVERSION=-igos-<release>`), so a release-bumped rebuild on a populated chroot orphans the prior release's tree instead of overwriting it, and downstream consumers pick ambiguously (the image phase's kernel symlink, the squashfs's wholesale `/usr/lib/modules` copy). Beyond `phase_validate` the same script also runs at every orchestrator phase entry from `kernel` through `iso`, inside the kernel phase driver (which additionally *prunes* superseded staged kernels before building), at the bootloader driver's prerequisites, and as squashfs Step 4.45 — so every entry point into the build asserts it. Resolve a failure by pruning the superseded release's module tree and vmlinuz; never ship a root with two staged kernels.
...
The two gates that most often fire when you add or change a package are **audit-coverage** and **tier validation**; the **source-tree-coverage** gate fires when a first-party package's `build.sh` reads an external in-tree source dir it doesn't declare. The rest of this doc walks each of these three; Gates 1–2 and 6–16 are structural/environmental and rarely need hand-resolution beyond reading their output (the resolution paths for Gates 6, 7, 8, 10, 11, 12 and 16 are fully described above).
