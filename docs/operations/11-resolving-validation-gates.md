...
| 12 | Licence identifiers (SPDX list membership) | `preflight-license-identifiers.py` | yes (exit 2 — never a quiet pass — when the bundled identifier file is missing/malformed, when the package tree is empty, or when the selected scope leaves nothing to check) |
