...
2. **Re-stage the complete evaluated corpus and republish** — the targeted-push procedure (`scripts/publish-repo.sh`; full mechanics + access in the publish runbook). The persistent-overlay model is retired: the archive directory passed to the publisher is a complete corpus whose bytes correspond to the build's chroot manifest in both directions. Replace the rebuilt package's prior archive in that complete set; remove a retired or renamed archive deliberately; then run the corpus gate.
   - Run `/usr/bin/bash /mnt/intergenos/scripts/publish-repo.sh --gpg-key NK1 --chroot-manifest /absolute/path/chroot-archives.sha256 --iso-sha256 /absolute/path/intergenos-r001.3.iso.sha256 --wiki-switching-page /absolute/path/wiki/src/start-here/switching.md`: it regenerates the whole index, checks public-document currency against the image checksum and generated index, and then enters the explicit signing hold. After the exact `sign` approval, NK1 signs the index with one PIN and one touch. The publisher delta-rsyncs only changed archives (`--link-dest` hardlinks the rest), atomically swaps `current/`, and appends the transparency-log entry.
