...
**Audience:** maintainers provisioning a new build VM (e.g., after hardware replacement, on a fresh workstation, or when rebuilding the known-good baseline snapshot from scratch).
...
3. Has masked apt background timers so `unattended-upgrades` doesn't run mid-build.
4. Has a build user with NOPASSWD sudo and SSH key auth from the workstation.
5. Has an empty `/mnt/igos/` directory (where each build's chroot will land).

This VM is the substrate every subsequent topic depends on. The final step ([topic 07](07-golden-builder-snapshot.md)) freezes it as a known-good baseline libvirt snapshot for rapid reverts between builds.
...
|---|---|
| `<username>` | Your Linux username (e.g., `builder`) |
...
```sh
# Keep scratch artifacts under a dedicated working directory, not /tmp.
...
`unattended-upgrades` will mutate host package state mid-build if left running, and the failure mode is non-deterministic: the build appears to work, then weeks later a package gets installed or upgraded during a long run and the chroot ends up subtly different. Mask both timers:

```sh
ssh <build-user>@<vm-ip> 'sudo systemctl mask apt-daily.timer apt-daily-upgrade.timer && sudo systemctl stop apt-daily.timer apt-daily-upgrade.timer'
ssh <build-user>@<vm-ip> 'systemctl is-enabled apt-daily.timer apt-daily-upgrade.timer'
...
```sh
ssh <build-user>@<vm-ip> 'sudo apt update && sudo apt install -y \
...
`unzip` is required by `scripts/build-intergenos-source-tarballs.sh` (line 83 hard-check) for extracting the `catppuccin-mocha-blue.zip` GTK theme and the GNOME extension UUID zips during the verify-sources phase. If it is missing, that phase fails fast with `FATAL: unzip missing on PATH`, so include it on every build-ready snapshot.

Verify the version-sensitive ones:

```sh
ssh <build-user>@<vm-ip> 'xorriso --version | head -1 && grub-mkstandalone --version'
```
...
```sh
ssh <build-user>@<vm-ip> 'mountpoint /mnt/intergenos && ls /mnt/intergenos | head'
...
```sh
ssh <build-user>@<vm-ip> 'sudo mkdir -p /mnt/igos && sudo chown <build-user>:<build-user> /mnt/igos'
```

The build orchestrator ([topic 02](02-running-the-builder.md)) populates `/mnt/igos/` as it runs.

### 8. (Optional but recommended) Run the validation checks before snapshotting

[Topic 07](07-golden-builder-snapshot.md) lists the validation queries to run before capturing a snapshot. Run them and confirm a clean state before invoking `virsh snapshot-create-as`.
...
- `ssh <build-user>@<vm-ip> id` → returns uid 1000 with `wheel`/`adm`/`sudo` group membership.
- `ssh <build-user>@<vm-ip> 'sudo -n true && echo OK'` → prints `OK` (NOPASSWD sudo works).
- `ssh <build-user>@<vm-ip> mountpoint /mnt/intergenos` → confirms virtiofs.
- `ssh <build-user>@<vm-ip> 'systemctl is-enabled apt-daily.timer apt-daily-upgrade.timer'` → `masked` for both.
- `ssh <build-user>@<vm-ip> 'command -v xorriso && xorriso --version | head -1'` → 1.5.6+.
...
- [Topic 07 — Snapshotting a baseline build VM](07-golden-builder-snapshot.md) — the immediate next step once this VM validates
- [Topic 02 — Running the builder](02-running-the-builder.md) — what runs inside this VM
- [`vm/cloud-init/README.md`](../../vm/cloud-init/README.md) — cloud-init template details and placeholder substitution
- [`vm/cloud-init/user-data`](../../vm/cloud-init/user-data) — the autoinstall config itself
- `scripts/build-vm-seed.sh` is referenced in `vm/cloud-init/README.md` but not yet authored; see the planned-work list in the [operations runbook README](README.md#status).
