...
### Step 2.8 — Build-identity stamp, guarded by the release-identity gate (decided 2026-09-14)

The launch name (`--iso-name`, persisted in `build/.iso-name`) is stamped into the image's `/etc/os-release` as `BUILD_ID` and `IMAGE_VERSION`, and the installer copies that tag onto every system it installs. Before the stamp, `scripts/check-release-identity.py` reads the four identity files as BUILT into the chroot (`igos-release`, `os-release`, `lsb-release`, `issue`) and refuses a chroot that still carries a previous `intergenos-base-files` — one installed system read R001.1 at the top of `cat /etc/*release` and a candidate tag of R001.2 at the bottom — and refuses a launch name that is not `intergenos-<release>.iso` (an optional re-mint ordinal; no `rc` prefix). After the stamp it reads the file back: `BUILD_ID` and `IMAGE_VERSION` must be that tag in the os-release character set. The tag is recorded in `build/.image-version`, and the ISO phase refuses to mint under a different name. A release build without a launch name is refused here (an `unnamed-<date>` tag is not a release identity); `UNSIGNED_TEST=1` media keep the chroot check and take the name as a warning.
