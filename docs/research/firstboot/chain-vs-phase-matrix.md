...
**Status:** DECIDED 2026-05-20 ~19:35Z-20:00Z. All 6 sub-decisions resolved as **Option A** in review. Implementation may proceed under the constraints + quality gates captured in this matrix and in the firstboot animation flow research notes.

**Author:** InterGenOS project — drafted as research-only matrix 2026-05-20 ~19:15Z; promoted to canonical research location after the decision.

**Scope:** this research matrix served as the decision frame for the walkthrough of the 6 sub-decisions named in the firstboot architecture rewrite.

---

## Decided verdicts (added 2026-05-20 post-walkthrough)

All 6 open questions at the end of this matrix resolved as **Option A** in review 2026-05-20 ~19:35Z-20:00Z:
...
**Smoothness quality bar (verbatim, Q3+Q4):** *"I spent a WEEK getting rid of tearing/odd glyphs/timing/etc - and it was FINALLY smooth as glass. It needs to STAY that way if it's moving to Python"* + *"if testing doesn't pan out- the entire thing is going back to C."*

**Design boundary (verbatim, Q5):** *"We're NOT changing this. Don't ask me another question that indicates you want to change anything about it."* — sweep count, timing, sweep rate, visual character, text content, font choice, fade-easing curves are LOCKED. Rewrite scope is mechanism (DRM/KMS → GTK4/Wayland) only.

The full verbatim quotes and the SMOOTHNESS QUALITY BAR are documented in the firstboot animation flow research notes.
...
**Directive sub-decision:** "REWRITE binary as GTK4-Wayland fullscreen window + SEQUENCE with intergen-welcome (chain-vs-phase sub-decision)".
...
| **Review surface** | Each component reviewable independently | Reviewer must hold both phases in mind simultaneously |
...
1. **Honest decomposition.** Two visually-distinct features (single-shot canvas animation + multi-page form wizard) are honestly two features. Forcing them into one binary respects the directive text but creates hidden coupling that bites the next refactor.
...
4. **Failure isolation is a security posture** — if the animation crashes (uncommon but possible on unusual GPUs / Wayland compositor edge cases), the welcomer MUST still fire so the user reaches a usable system.
...
The 6 questions below are the original scope-prep open questions that the walkthrough on 2026-05-20 ~19:35Z-20:00Z resolved. See [Decided verdicts](#decided-verdicts-added-2026-05-20-post-walkthrough) table at top of document for the ratified outcomes. Preserved here as scope-prep historical-context for future readers tracing the decision provenance.
...
6. **Existing intergen-firstboot binary disposition?** The directive says DELETE `assets/intergen-firstboot-drm/intergen-firstboot.service`. Should the C source at `assets/intergen-firstboot-drm/firstboot-drm.c` ALSO be deleted, or kept for archival/reference value? The non-drm `assets/intergen-firstboot/firstboot.c` (SDL2 prototype, 489 lines) — same question.

---

## Cross-references

- Directive context: maintainer directive 2026-05-20T18:29:14Z
- Primary-source greenlight: firstboot animation flow research notes
- Review discipline: review-before-save discipline applied at ratification
...
- Password greeter (NOT IN SCOPE — different artifact): `installer/data/intergenos-first-boot-greeter.service` (hardened at `bb43a724`)
...
**Next step:** matrix is DECIDED + closed as a research artifact; the Python rewrite work proceeds per the decision-queue verdicts on (a) implementation ownership, (b) test-plan-first sequencing per the smoothness QA hard gate, and (c) hardware-testing protocols on the reference DRM hardware before the Python binary supersedes the existing C/DRM implementation. The rewrite implementation is subject to the D-009 8-item checklist + review on the commit SHA when it lands.
