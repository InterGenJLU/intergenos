...
| Ampere | RTX 30xx (incl. the **3070 Ti test bed**), A-series | 515+ | Covers Zephyrus M16 test bed |
...
- Zephyrus M16 test bed (RTX 3070 Ti, Ampere): supported ✓
...
### Option A: Fine-grained split (InterGenOS-preferred)
...
- User can inspect the fetch helper, see exactly what URL is hit and what sha256 is verified. User control preserved.
...
- Requires taking a position on EULA redistribution (NVIDIA's EULA permits it with terms — needs legal review, outside scope of this research doc).
...
2. User control: user decides whether to accept the userspace fetch. Headless servers or compute-only boxes may want kernel modules without GL libraries at all (run pure CUDA against libcuda as an overlay).
...
**Day 2 validation:** verify `modinfo -F sig_hashalgo nvidia` reports `sha256` and `modinfo -F signer nvidia` reports our MOK CN. Worth adding to Class 1 harness for NVIDIA-present systems.
...
The Zephyrus M16 test bed has Intel integrated + RTX 3070 Ti. BIOS MUX modes:
...
When a direction is approved, **package template work becomes a separate task** (likely mid-week after owner returns to desk, as originally queued). This doc is the upfront research so that implementation goes quickly and doesn't rediscover these constraints.
