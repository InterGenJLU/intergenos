...
> **This is a historical 2026-04-09 design proposal, kept as an archival record only.**
> The `--package <name>` flag proposed below was **never implemented** — the builder's
> single-package primitive is `--only <name>` (see `igos-build/__main__.py`). The build
> procedure described here is also **wrong by current practice**: single-package builds
> run **inside the chroot** (the toolchain exists only in `/mnt/igos`) and require
> refreshing the chroot's recipe copy first.
>
> **Canonical, current single-package references:**
> - [`docs/operations/08-adding-packages.md`](../../operations/08-adding-packages.md) — authoring + test-building a new package (step 5 = the `--only` surgical build)
> - [`docs/operations/02-running-the-builder.md`](../../operations/02-running-the-builder.md) — "Build a single package" + the chroot-recipe-sync prerequisite
> - [`docs/operations/10-iteration-resume-builds.md`](../../operations/10-iteration-resume-builds.md) — `--skip-built` template-hash + resume semantics

# Single-Package Build Mode — Design Proposal

**Date:** April 9, 2026  
**Status:** SUPERSEDED (was: Proposal — awaiting owner review). See the banner above.
