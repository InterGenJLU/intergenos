...
**Date:** 2026-04-08
**Status:** Approved
**Based on:** Flathub 2025 data, Snap Store data, Arch Community Survey 2024, DistroWatch top 10, default app analysis across 6 major distributions, a daily-use list from the development workstation

---

## Guiding Decisions

1. **No Snaps or Flatpaks.** InterGenOS builds from source. We don't have the version mismatch problem they solve. They add bloat, duplicate libraries, and hide how the system works — contrary to the goal of a system users can understand, modify, and trust.

2. **Proprietary apps via install helpers.** `pkm install-helper <name>` downloads vendor binaries. No proprietary code in our tree.

3. **Ship everything a user needs on day one.** Mint/Pop!_OS model, not Arch model. The system should work without hunting for basic tools — putting users in control from the moment they log in.
...
| # | Application | Category | Evidence |
|---|-------------|----------|----------|
| 19 | Timeshift | System snapshots | Default in Mint, user control: recover from mistakes |
