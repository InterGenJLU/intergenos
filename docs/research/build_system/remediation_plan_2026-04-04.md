...
**The project's core principle demands it:** *"InterGenOS exists to put the user in control of their own machine. Every design decision, every default, every included component must serve this purpose: giving people a system they understand, can modify, and can trust."*
...
Verify in parallel batches (2 batches of 14) against LFS 13.0 Ch. 5-7:
...
Verify in parallel batches (5-6 batches of ~18-20):
...
Small tier — verify against BLFS 13.0:
- Version match
...
Audit in 7-8 parallel batches (~22-25 packages each) against BLFS 13.0. For each package:
