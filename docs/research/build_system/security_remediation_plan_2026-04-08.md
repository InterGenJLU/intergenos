...
The driving motivation: InterGenOS currently operates on **implicit correctness** — builds succeed because we happen to run things in the right order, not because the system enforces it. InterGen Sentinel integration demands **enforced correctness**.
...
- build docs — document the naming requirement
