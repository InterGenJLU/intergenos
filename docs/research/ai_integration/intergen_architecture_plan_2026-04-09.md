...
InterGen Daemon (Python, systemd user service)
  ...
  ├── LLMRouter (llama.cpp + quality gates + cloud fallback)
...
## "Phone-A-Friend" (Frontier/Cloud Escalation) — Cloud API Escalation

InterGen knows its limits. When the local model can't deliver, it escalates to the user's configured cloud provider.
...
- Sentinel security scanning (cloud provider, user-configured)
...
**New code:** daemon (D-Bus), hardware detector, model_manager, llama_manager, persona, security-scanner guard, GTK4 panel app (~800 lines), GNOME extension (~400 lines)
...
- 4 packages: llama-cpp → intergen → intergen-gnome-extension, intergen-sentinel
