...
Handles "remember that...", "what were we working on?", "forget about...". Explicit storage only — no passive fact extraction (user controls what's stored).
...
- **User-control principle** — "InterGenOS exists to put the user in control of their own machine." Every design decision serves user transparency and control.
...
- R10 grader (original): keyword-based, caught surface issues, missed 50+ quality failures.
  Manual review of R10 exposed this. Documented in `round10_full_review_manual.txt`.
- R14 grader v2: 5 new auto-assertions, category-aware skipping, broader denial-phrase
  detection. Calibrated against manual labels on R10.
...
Manual review of R10 flagged 50+ grader-PASS responses as actually poor;
...
class ConversationRouter(RouterInterface):
    ...
    def _try_memory(self, user_input: str) -> RouteResult:
        ...
        # Skipped for now — only explicit storage (user controls what's stored)
