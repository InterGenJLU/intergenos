...
New packages (ai tier), build order:
  ...
  6. intergen-sentinel  [custom, Python, depends on 5] ← included
...
### 6. intergen-sentinel (included in standard build)
- **Style:** custom, source: []
- **Deps:** intergen, python
- **Installs:** Helper that pip-installs `anthropic` SDK + the security-scanner module
...
- `packages/ai/intergen-sentinel/{package.yml,build.sh}`
