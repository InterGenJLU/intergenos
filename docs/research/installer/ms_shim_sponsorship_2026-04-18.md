...
> **📜 HISTORICAL RESEARCH SNAPSHOT (decided 2026-08-16).** This is a dated
...
> tree itself and the current `docs/`. See also the dated correction notice below.
...
> **⚠️ CORRECTION (2026-06-11) — a load-bearing claim in this doc is WRONG.** This research stated that "Path B (shim-review) → Red Hat batches to Microsoft" so InterGenOS needs **no EV certificate and no Microsoft account**. That is incorrect, verified against primary sources (the shim-review `README` template requires disclosing YOUR EV cert issuer/subject; Microsoft's UEFI signing requirements make an EV cert mandatory; recent NEW-vendor acceptances — Ziperase #477, Freexian #435 — each obtained their OWN EV cert + Microsoft Partner Center account). **shim-review waives ONLY the annual third-party security audit — NOT the Microsoft account or the EV cert; there is no "Red Hat batches on our behalf" mechanism.** The distro itself registers a Microsoft Hardware Dev Center account, signs the `.cab` with its OWN EV code-signing certificate, and uploads it. **Acquiring an org-validated EV cert (hardware-token-stored) + a Microsoft Partner Center account is a real prerequisite for our own-shim path.** The "Zero direct $ / Red Hat absorbs EV-cert" lines below are superseded by this correction; the avenue analysis (Path A/B/sponsorship/piggyback) and the timeline figures remain valid.
