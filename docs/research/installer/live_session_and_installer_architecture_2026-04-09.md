...
> **⚠️ 2026-05-18 RESOLUTION BANNER — read before this doc.** This is an *options-analysis* document that enumerates how other distros approach the live-session / initramfs problem, including dracut and mkinitcpio. **Every mention of dracut and mkinitcpio in this document is historical analysis of other-distro approaches. InterGenOS does NOT use dracut. InterGenOS does NOT use mkinitcpio. Both are RATIFIED-AGAINST.**
>
> The ratified InterGenOS approach (see §4 / §9 below + `live_session_and_installer_FINAL_2026-04-10.md`):
> - **Live ISO initramfs:** custom 100-line `/init` script + statically-linked busybox (LFS way; no framework).
> - **Plain installs (installed system):** no installed-system initramfs — all storage/FS drivers built into the kernel `=y`; kernel boots directly via UKI envelope (D-005).
> - **LUKS installs (installed system):** tiny custom busybox-static FDE-initramfs bundled INSIDE the UKI envelope (D-001 I-D); generated by `installer/init/build-fde-initramfs.sh`.
>
> Multiple ratifications: 2026-04-08/09 (PARTUUID + no-installed-system-initramfs), 2026-04-10 FINAL (custom `/init`, **"No casper, no dracut, no mkinitcpio"**), 2026-05-05/06 Q-INIT (commits `77b0b453` / `cfcdc82e` / `a12216a5` explicitly rejected dracut-uefi), narrowed by D-001 + D-005 2026-05-18.
...
- Aligns with project goals: transparent, understandable, no hidden magic
...
| GUI installer proposal | ✅ Designed | GTK4 Python, 4 screens, uses same backend |
