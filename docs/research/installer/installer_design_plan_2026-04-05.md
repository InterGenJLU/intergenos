...
A custom installer gives us full control over the InterGenOS-specific experience (AI tier detection, transparency about what is installed and why) while reusing `pkm` as the engine.
...
- Explain what InterGenOS is and its user-control philosophy
...
- ~~Future: LVM, LUKS encryption~~ **LUKS SUPERSEDED-BY 2026-05-18 D-001:** LUKS-at-install promoted to v1.0 baseline (opt-in passphrase-only LUKS2). LVM remains future-scope until a new directive ratifies.
...
- ~~Encryption (LUKS2)~~ **SUPERSEDED-BY 2026-05-18 D-001:** LUKS2 promoted to v1.0 baseline (opt-in passphrase-only). TPM-sealed unlock + FIDO2 are v1.0 EXPERIMENTAL.
