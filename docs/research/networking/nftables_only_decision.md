...
- [x] Audit `packages/desktop/` for any package that depends on iptables-legacy specifically (vs. accepting either backend). — Closed 2026-07-28: the sole desktop consumer is NetworkManager; our iptables builds `--enable-nftables`, and no legacy-specific edge exists.
- [x] Update `vision/README` (when it exists) to document the firewall posture for end users. — Closed 2026-07-28: the end-user firewall posture is documented in the wiki (`security/network-security.md`, default-deny + nftables-only, explicit).
- [x] Consider whether `iptables` package belongs in `extra` tier (opt-in) rather than `desktop` (default) — the iptables-nft compat shim is small but adds nothing for users who use `nft` directly. Defer until usage data justifies. — Resolved 2026-07-28: stays in desktop, consumer-anchored (shipped NetworkManager runtime-depends on it; the usage question is answered).
- [x] Document the `nft` CLI as the preferred interface in InterGen's system-help responses. — Done 2026-07-28: the networking help corpus's firewall entry answers nft-first and now states the preference explicitly for iptables-shaped questions.
