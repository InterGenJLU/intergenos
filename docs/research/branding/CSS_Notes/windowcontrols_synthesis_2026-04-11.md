...
**Date:** April 11, 2026
**Status:** Diagnosis confirmed by three independent reviews
...
- **Patch Nautilus's `.ui` file** to set `hexpand="False"` on the windowcontrols box (requires building Nautilus from source — treating this as an upstream workaround rather than a real fix)
...
Implementing Reviewer C's Option A (image-as-entire-visible-button) synthesized with Reviewer A's image padding values and Reviewer B's explicit dimensions.
