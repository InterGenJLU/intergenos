...
### Discrepancy with the project instructions file
The project instructions file still says "458 package templates" — stale by 187. Worth updating during next docs pass.
...
- `chmod 777`: **none**
- `rm -rf /`: **none**
- Hardcoded secrets/tokens: **none** (two default passwords in `create-image.sh:380,391` are env-var-overridable placeholders, already reviewed in external-audit commit `c0454b2`)
...
- `c0454b2` — external security-audit fixes (PARTUUID, eval, firewall, passwords)
...
### Project instructions drift
The project instructions file says "458 package templates". Actual: 645. Should be updated next docs pass.
...
The FLUX branding plan on the home drive includes logo, boot splash, greeter backgrounds, GRUB background, Plymouth, and wallpaper prompts. Assessment: **none of the 82 generated match what we're looking for** — iteration continues.
...
3. **Update** the project instructions file package count from 458 → 645
