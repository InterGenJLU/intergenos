...
> **SUPERSEDED.** The graduation this roadmap sequences toward (§2 Step 8) has
> completed: the 39Q skeleton graduated to
> [`docs/shim-review-submission.md`](../../shim-review-submission.md), which is
> now the live, maintained submission document (current PR-open target
> 2026-05-29, superseding this doc's 2026-05-15 target). Kept here as
> historical record of the sequencing work between the B2 build and PR-open.
...
- the v1.0 documentation-scope §6.2 sequencing context
- B2 reproducibility harness research (AI-assisted research, 2026-05-01)
- GATED-item paste-ready templates (AI-assisted research, 2026-05-01)
- C6 PIV-toolchain research (AI-assisted research, 2026-05-01 — V2 PASS, untested-against-hardware)

**Purpose:** sequence the work between "B2 build is runnable" and "39Q skeleton ready for shim-review PR-open at 2026-05-15." Internal roadmap — owner runs it, maintainers support.
...
C6 (PIV slot 9c keygen) ──→ vendor cert public part exists
   ...
   │       │
   │       ├──→ verify-b2-reproducibility.sh ──→ Q22 reproducibility verdict
   │       │       │
...
**Current blocker:** C6 (PIV slot 9c keypair generation) outstanding per ceremony-day toolchain gap. The toolchain research (V2 PASS) recommends `piv-tool` from opensc as the candidate primary. Owner Nitrokey #4 smoke-test is the gating step before C6 re-attempt.
...
**Lead:** owner; maintainers support
**Inputs:** Nitrokey #4 (factory-fresh spare), the research doc, opensc + piv-tool packages

Owner runs the recommended command sequence on NK#4 in a non-air-gap setting to validate the toolchain works on real Nitrokey 3 hardware:
...
**Failure-mode handling:** if `piv-tool` doesn't work as researched, fall back to `nitropy nk3 piv` (the #2 candidate) or `OpenSSL+libp11` (the #3 candidate). Each fallback adds ~30 min to this step.

### Step 2: C6 re-attempt in air-gap (executes the validated toolchain)

**Time estimate:** 60-90 min (Tails air-gap session, owner runs)
**Lead:** owner; maintainers support via runbook
...
**Lead:** maintainer or owner runs `docker build`; maintainers support
**Inputs:** vendor cert PEM from Step 2 (cert public-part embeds in the shim binary), `docker/shim-build/Dockerfile` at master tip

**Pre-flight:**
- The batch-5 forge-installer fixes have landed (✓ at master `15ce475`); independent of B2 but should be merged before B2 run for clean master state
...
### Step 4: Run the verify-b2-reproducibility.sh harness

**Time estimate:** Step 3 runs again on a different host + Step 4's verify ~10 min
**Lead:** a maintainer runs on a second host; another maintainer compares
**Inputs:** two independent `shimx64.efi` artifacts from Step 3 (one per host), the `verify-b2-reproducibility.sh` script (currently at home-drive on Ubuntu; would graduate to `scripts/verify-b2-reproducibility.sh` if landed in repo)
...
Expected output: 6/6 PASS. The 6 checks: tarball sha256, shim binary, vendor cert, commit SHA, SBAT, PE metadata.

**Verification (Q22 fill):** if 6/6 PASS, Q22 answer becomes "Yes — bit-for-bit reproducible across N independent hosts; harness output sha256 matches across hosts." If <6/6 PASS, identify which check fails and apply the reproducibility-leak fixes (L1 thread-race + L2 apt versions are the highest-impact).

**Updates to repo:**
- `docs/research/shim_review/README_draft_skeleton.md` Q14, Q22, Q23, Q25, Q29, Q30 fill from the prepared paste-ready templates using actual harness outputs
...
**Lead:** owner; maintainers support with structure inventory
...
**Lead:** maintainers; per Q38's own design, "starting 2026-05-04"
**Inputs:** open shim-review PRs from non-major-distro submitters

Per Q38's already-written plan: a maintainer picks ≥2 open shim-review PRs, posts substantive review comments. Records URLs in Q38's body.

### Step 8: Pre-PR-open final pass + graduation

**Time estimate:** 60-90 min per v1.0-doc-scope §6.2 step 5
**Lead:** maintainers; owner reviews
...
| Q22 | GATED on B2 + harness | Step 4 | harness 6/6 PASS verdict |
...
**Summary:** 23 questions FILLED, 6 N/A, 7 GATED on B2, 2 GATED on C6, 1 owner action (Q9 fork), 1 secondary-maintainer action (Q7 Ethan), 1 secondary-maintainer ongoing (Q38 reviews).
...
| # | Risk | Mitigation |
|---|---|---|
| R1 | NK#4 smoke-test fails — the piv-tool sequence doesn't work on real Nitrokey 3 hardware | Fall back to nitropy nk3 piv (the #2 candidate) or OpenSSL+libp11 (the #3 candidate). Adds 30-90 min per fallback attempt. |
...
| R4 | The reproducibility harness reports <6/6 PASS | L1 + L2 are already identified as primary fixes. Apply those, re-run. If new leaks surface, iterate. |
...
| R8 | The batch-5 fixes (already landed at 15ce475) cascade into Forge-installer instability that affects the bare-metal install validation needed for the truth-doc | Independent of B2 + C6 chain; maintainers handle. Doesn't block 39Q completion. |
...
- the v1.0 documentation-scope §6.2 — broader v1.0 sequencing context
