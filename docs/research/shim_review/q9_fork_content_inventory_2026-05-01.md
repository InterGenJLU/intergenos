...
> **SUPERSEDED.** The `20260515` branch tag and the "current 39Q at master tip"
> reference below (`README_draft_skeleton.md`) both reflect the pre-graduation
> state. The submission text graduated to
> [`docs/shim-review-submission.md`](../../shim-review-submission.md) under the
> `intergenos-shim-x64-20260529` branch tag. Kept here as historical record of
> the fork-content planning work.
...
- the v1.0 documentation-scope graduation plan (§3.1) — an internal planning note, not published in this repo
...
| Build logs | Docker buildkit output + the verify-b2-reproducibility.sh harness output | GATED on B2 + harness run | Steps 3-4 |
...
- Reproducibility verified (6/6 PASS) → see roadmap Step 4
...
7. **Copy build logs into `logs/` subdir** (Docker buildkit output + harness output, 5 min)

8. **Graduate `docs/research/shim_review/README_draft_skeleton.md` → fork's `README.md`** with all GATED items filled per the post-B2 outputs (per the roadmap Step 4 paste-ready templates from 2026-05-01T17:23:55Z deliverable)
...
**Total time estimate (assuming pre-conditions hold):** ~60-90 min owner work + 30-45 min maintainer support (review, etc.)
...
└── logs/
    ...
    ├── verify-reproducibility_<timestamp>.log  # harness output (host A vs host B comparison)
...
- the v1.0 documentation-scope graduation plan (§3.1) — broader graduation context (internal planning note, not published in this repo)
