...
The founding goal of InterGenOS is simple:
...
the InterGenOS model mirror (with the model author's distribution,
HuggingFace, as a fallback), which reveal your IP and User-Agent to
whichever source serves the file; (c) **explicit user-invoked network
actions** (the InterGen assistant's `web_search`, `weather`, etc.;
your web browser; any package you install that makes network calls),
which behave according to those tools' own network behavior; and
(d) the opt-in **Phone-A-Friend (Frontier/Cloud Escalation)**
feature, which sends conversation content to a cloud AI provider
that you configure, and only after you consent. There is **no
telemetry**, no analytics ping, no phone-home, no usage reporting,
no crash reporting transmitted off your machine, and no shadow
background traffic. We do not sell, share, or transfer any data we
do not have.
...
- **No "improved by your data" model training.** Nothing you do on
  InterGenOS — including your conversations with InterGen — is used
  by InterGenJLU to train, fine-tune, or otherwise improve any model.
  We do not have your conversations. (If you opt in to the
  Phone-A-Friend feature in §5.5 and consent to a send, the cloud
  provider you chose receives that content under its own terms, which
  may differ; that is a choice you make and control.)
...
TLS prevents the request body and response body from being read by a
network observer in transit between you and the mirror, on the
assumption that the certificate chain holds: an adversary able to
obtain or mis-issue a trusted certificate for `repo.intergenos.org`,
or to add its own certificate authority to your trust store, can
terminate the connection and read both. Package integrity does not
rest on TLS — `pkm` verifies the signed index and each archive's
hash independently, which is why the mirror design treats TLS as
transport-only (see [docs/mirror/design.md](docs/mirror/design.md)).
...
downloads them at first run, **only if you launch InterGen**.

**Where from.** InterGen fetches Qwen3.5 GGUF model quantizations
and the nomic-embed-text-v1.5 embedding model from two possible
sources, tried in order:

1. The **InterGenOS model mirror** at `repo.intergenos.org/models`,
   which is the authoritative source and hosts the exact
   integrity-pinned files we validate. This is the same mirror
   described in §5.1, operated under InterGenJLU's name.
2. **HuggingFace** (`huggingface.co`) as a fallback if the mirror
   is unavailable. HuggingFace is a third-party service operated by
   **Hugging Face, Inc.** (United States); its privacy policy
   applies to data it receives.

Every downloaded file is verified against a SHA-256 pin shipped with
the InterGen package, regardless of which source served it, so a
drifted or substituted file is rejected. When InterGen requests a
model file, your IP address and User-Agent reach whichever source
serves it.
...
The first time you launch InterGen, you will see a confirmation
prompt before any network access, disclosing (a) the model that will
be downloaded, (b) the source it will be downloaded from, (c) its
download size, and (d) the fact that this is a network-crossing
point. You can decline; if you decline, InterGen does not start.
...
**What we do with it.** When the InterGenOS model mirror serves the
file, the same handling as §5.1 applies: the mirror's access log
records your IP, User-Agent, time, and the path requested, and we do
nothing with it beyond operating the mirror. When the HuggingFace
fallback serves the file, InterGenJLU is not in the path at all — the
download is between your machine and HuggingFace. Either way, we do
not build a download profile and we do not correlate model fetches
to identify users. You can verify this by inspecting the network
calls or reading the [InterGen source](packages/ai/intergen/).

**Your control.**
- Decline at the acceptance prompt (InterGen does not start).
- Decline and uninstall InterGen entirely (`pkm remove intergen`).
- Run your own model mirror and point InterGen at it (the same
  self-hosting path described in §5.1).
- Pre-stage models offline (copy them onto your machine by any means)
  and tell InterGen to use the pre-staged files via the
  `INTERGEN_MODEL_PATH` / `INTERGEN_EMBED_MODEL_PATH` settings instead
  of fetching.
...
### 5.5 Phone-A-Friend (Frontier/Cloud Escalation)

**What.** InterGen includes an opt-in feature called **Phone-A-Friend
(Frontier/Cloud Escalation)**. When a request is multi-step, sensitive,
or outside what the local model can do well, InterGen can offer to
forward the relevant conversation content to a more capable cloud AI
model that you have configured. If the request goes ahead, that content
is sent to a third-party AI provider.

**Off by default; you choose the provider.** No cloud provider is
configured out of the box, so escalation cannot send anything until you
deliberately set one up. InterGenOS supports several providers you may
choose from — for example Claude (Anthropic), Gemini (Google), CoPilot
(Microsoft), ChatGPT (OpenAI), Grok (xAI), and DeepSeek — and you supply
your own credentials for whichever you pick. Those credentials are
stored locally on your machine.

**Who decides each send.** You do. In the default mode, InterGen
**offers** to escalate and waits for your consent before anything
leaves your machine; it shows you what would be sent first. You can
disable the feature entirely (escalation mode `never`), or change the
behavior to suit you. The cloud connection is refused over anything but
an encrypted (TLS) transport, and a security scan on derived content
blocks any secrets from being forwarded.

**Who sees what.** Whatever content you consent to send reaches the
cloud provider you configured, and that provider's privacy policy
governs what it does with the data. **InterGenJLU is not in this path:**
the connection is directly between your machine and your chosen
provider. We do not see, log, or proxy the request.

**Your control.**
- Leave it off (no provider configured — the default).
- Set escalation mode to `never` to suppress every offer.
- Review what would be sent before each send, and decline any of them.
- Choose, change, or remove your provider and credentials at any time.
...
| InterGen conversation history | `$XDG_DATA_HOME/intergen/sessions/` (per user, one JSON file per session) | Bounded by user-controlled retention setting (default: indefinite until you delete) |
...
This notice was authored 2026-05-18 as the initial privacy notice
for the InterGenOS v1.0 release.
