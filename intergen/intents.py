...
def _register_system_info(matcher: SemanticMatcher) -> None:
    ...
    matcher.register_keyword_pattern(
        "system_info",
        [
            r"^(?:how much|what(?:'s| is))\s+(?:my\s+)?(?:disk|storage|space|memory|ram|cpu|uptime)",
            r"^(?:show|check|display)\s+(?:my\s+)?(?:disk|storage|memory|ram|cpu|system)\s+(?:usage|info|space|status)",
            r"^(?:free|df|du|top|htop|uname|uptime)\b",
