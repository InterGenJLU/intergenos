...
Wiring (AI-2, audit 2026-05-29 — SUPERSEDES the original Q8 per-tool answer):
wrapping happens at the CENTRAL tool-dispatch chokepoint
(`tool_registry.execute()`), which wraps any successful result from a tool in
`INGRESS_TOOLS_V1` before it re-enters the LLM context. The original Q8 design
(2026-05-19) wrapped per-tool at tool-result construction; that was never
actually wired (the module was dead code) AND let the ingress watch-list drift
out of sync with the tools (AI-13). The central chokepoint keys off the SAME
`INGRESS_TOOLS_V1` set that drives the §5.1 watermark, so the two cannot
diverge. Source attribution is derived centrally from the tool name + arguments
(`tool_registry._ingress_source_attribution`).
