...
REQUIREMENTS = [
    ...
    Requirement(
        name="Python",
        # 3.12 floor: the build system's extraction path uses the PEP 706
        # tarfile 'data' filter, available from 3.12 — an older interpreter
        # passes a lower floor and then fails mid-build.
        min_version="3.12",
    ...
    Requirement(
        name="setpriv",
        min_version="2.30",
        command="setpriv --version | head -1",
        version_regex=r"(\d+\.\d+(?:\.\d+)?)",
        notes="build-intergenos.sh launches the toolchain build through "
              "setpriv (privilege drop with no PAM and no cgroup migration, "
              "so children die with the transient unit); absent on hosts "
              "whose util-linux built without libcap-ng — first hit on the "
              "2026-07-17 self-hosted lane bring-up",
    ),
    Requirement(
        name="Tar",
        min_version="1.22",
    ...
    Requirement(
        ...
        version_regex=r"qemu-nbd (\d+\.\d+(?:\.\d+)?)",
    ...
    # ISO-pipeline + signing-ceremony prereqs (phase_squashfs → phase_iso and
    # the two operator signing pauses). A host that can build every package
    # but lacks these fails at the very end of the pipeline — or worse, at
    # ceremony time with the operator waiting. Prove the whole stack up front.
    Requirement(
        name="mksquashfs",
        min_version="0",
        command="mksquashfs -version 2>&1 | head -1",
        version_regex=r"version (\d+\.\d+(?:\.\d+)?)",
        notes="squashfs-tools — builds the live filesystem (phase_squashfs)",
    ),
    Requirement(
        name="veritysetup",
        min_version="0",
        command="veritysetup --version 2>&1 | head -1",
        version_regex=r"(\d+\.\d+(?:\.\d+)?)",
        notes="cryptsetup — dm-verity hashtree over the squashfs (boot trust path)",
    ),
    Requirement(
        name="xorriso",
        min_version="0",
        command="xorriso --version 2>&1 | head -1",
        version_regex=r"xorriso (\d+\.\d+(?:\.\d+)?)",
        notes="xorriso — assembles the hybrid UEFI ISO (phase_iso)",
    ),
    Requirement(
        name="sbsign",
        min_version="0",
        command="sbsign --version 2>&1 | head -1",
        version_regex=r"(\d+\.\d+(?:\.\d+)?)",
        notes="sbsigntool — ceremony #2 signs GRUB + the 3 UKIs (PIV slot 9c)",
    ),
    Requirement(
        name="sbverify",
        min_version="0",
        command="sbverify --version 2>&1 | head -1",
        version_regex=r"(\d+\.\d+(?:\.\d+)?)",
        notes="sbsigntool — post-sign verification against the vendor cert",
    ),
    Requirement(
        name="gpg",
        min_version="2.0",
        command="gpg --version | head -1",
        version_regex=r"(\d+\.\d+(?:\.\d+)?)",
        notes="gnupg — ceremony #1 detached-signs the archive manifest (OpenPGP card)",
    ),
    Requirement(
        name="pcscd",
        min_version="0",
        command="pcscd --version 2>&1 | head -1",
        version_regex=r"(\d+\.\d+(?:\.\d+)?)",
        notes="pcsc-lite — smart-card daemon for the signing token; must also be "
              "RUNNING at ceremony time (pcscd.socket)",
    ),
]
...
def check_symlink(path: str, expected_target: str, remote: Optional[str] = None) -> tuple[bool, str]:
    """Check that path resolves to (or behaves as) the expected tool.

    Shape check first: the resolved path names the expected target — the
    hand-made-symlink convention on prep'd build hosts. Behavior fallback:
    some hosts satisfy the requirement with the tool's OWN wrapper instead
    of a symlink (bison installs /usr/bin/yacc as a shell script that execs
    `bison -y`), so when the shape test misses, accept the path if its
    --version output names the expected implementation. The requirement is
    "yacc IS bison", not "yacc is spelled as a symlink".
    """
    ...
    vcode, voutput = run_command(f"{path} --version 2>/dev/null | head -1", remote)
    if vcode == 0 and expected_target.lower() in voutput.lower():
        return True, f"{path} is {voutput.strip()} (wrapper, not symlink)"
...
def check_library_consistency(remote: Optional[str] = None) -> tuple[bool, str]:
    ...
    A probe ERROR is not a probe FINDING: run_command returns sentinel
    strings on timeout/exception ("(command timed out)" / "(error: ...)"),
    and bool(output) previously counted those as "library present" — under
    heavy host I/O (a concurrent full-chroot verify sweep) the first find
    timed out cold-cache and produced a phantom INCONSISTENT verdict
    (ge9b-02, 2026-07-12). Fail loud on any probe error instead.
    """
    ...
    for lib in libs:
        ...
        code, output = run_command(cmd, remote)
        if code != 0 or output.startswith("(command timed out)") or output.startswith("(error:"):
            return False, (
                f"PROBE ERROR while searching for {lib} (rc={code}, output={output!r}) "
                f"— cannot certify consistency; re-run when the host is not under heavy I/O"
            )
...
def main():
    ...
    for req in REQUIREMENTS:
        ...
        if code != 0 and not output:
            ...
            # Check minimum
            if req.version_regex and not match:
                # Fail-closed: a declared version pattern that does not match
                # the command's output is a FINDING (tool absent -> shell
                # error text in output, or upstream changed its banner) —
                # never a silent pass. Caught live 2026-07-18: an absent
                # pcscd's "command not found" sailed through as OK.
                status = "FAIL"
                detail = "(version output unrecognized)"
                failed += 1
            elif match and not version_ge(version, req.min_version):
                status = "FAIL"
