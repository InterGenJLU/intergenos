...
PHASE_ORDER = [
    ...
    "ch10",
    "desktop",
    # Reordered 2026-07-21 (training-stack wave): extra and compute build
    # BEFORE ai so the ai-tier GPU-native builds (torch family) can consume
    # the compute SDKs and extra-tier libs at build time. Mirrors the
    # run_phase order in build-intergenos.sh — keep the two in lockstep.
    "extra",
    "compute",
    "ai",
    "bootloader",
...
def scan(repo: Path) -> tuple[dict, list[dict], dict]:
    ...
    for phase in PHASE_ORDER:
        ...
        if not script.is_file():
            # A phase script may legitimately not exist (e.g., bootloader is
            # assembled by build-intergenos.sh directly) — but the skip is
            # NAMED in the summary, never silent: a scan that quietly covered
            # fewer scripts than the phase order reads as full coverage.
    ...
    if missing_scripts:
        print(f"Phase drivers absent (named skip): {', '.join(missing_scripts)}")

    duplicates = {n: locs for n, locs in pkg_index.items() if len(locs) > 1}

    findings: list[dict] = []

    if not any(script_pkgs.values()):
        # Zero packages parsed across every phase driver: nothing was
        # validated. Exit 0 here certified an empty scan (wrong --root, all
        # drivers absent) — the vacuous-pass shape.
        findings.append({
            "type": "EMPTY-SCAN",
            "detail": (f"zero run_package lines parsed across "
                       f"{len(PHASE_ORDER)} phase drivers under {scripts_dir} "
                       f"(missing: {', '.join(missing_scripts) or 'none'}) — "
                       f"an empty scan validates nothing"),
        })
...
def emit_summary(script_pkgs: dict, findings: list[dict], duplicates: dict, verbose: bool) -> None:
    ...
    for t in ("EMPTY-SCAN", "DUPLICATE-PACKAGE-NAME", "SAME-SCRIPT-VIOLATION",
