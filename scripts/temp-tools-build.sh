...
# Runs OUTSIDE the chroot as the build user; IGOS_TRACE_ROOT threaded through
# env -i by the orchestrator (phase_toolchain).
# shellcheck disable=SC1091
[ -f /mnt/intergenos/scripts/lib/trace.sh ] && source /mnt/intergenos/scripts/lib/trace.sh
[ "${IGOS_TRACE_LIB_LOADED:-0}" = "1" ] && trace_init "tier-temptools"
...
run_pkg() {
    ...
    trace_pkg_capture --pkg "$name" --version "$version" --tier temptools --phase all --rc "$rc" --duration-ms "$(( elapsed * 1000 ))" --log "$pkg_log" --cmd-file "$1"
...
# This block MUST stay in lockstep with packages/toolchain/gcc-pass2/build.sh —
# the recipe file is NOT executed here (GE-01 launch-5 halt: the wave-0 multilib
# flags landed in the recipe while this executed copy still said --disable-multilib,
# so pass2 shipped no 32-bit libgcc and lib32-glibc failed at -lgcc).
