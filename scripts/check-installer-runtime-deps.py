...
def scan_python(installer_dir: Path) -> set[str]:
    ...
    Excludes installer/tests/ — those are dev-machine test harnesses
    (e.g., secboot_vm_profile.py invokes virsh / virt-install / swtpm
    for SB validation VMs), NOT runtime installer code. Including them
    causes false-positive M-002 violations for binaries that aren't
    even meant to ship in the live ISO.
    """
    found: set[str] = set()
    if not installer_dir.exists():
        return found
    for py in installer_dir.rglob("*.py"):
        # Skip test harnesses — not runtime installer code.
        if "tests" in py.relative_to(installer_dir).parts:
            continue
        try:
