...
# Runs OUTSIDE the chroot as the build user; IGOS_TRACE_ROOT was made
# writable by the orchestrator (phase_toolchain) and threaded through env -i.
...
build_package() {
    ...
    trace_pkg_capture --pkg "$pkg_name" --version "$pkg_version" --tier toolchain --phase all --rc 0 --duration-ms "$(( elapsed * 1000 ))" --log "$pkg_log" --cmd-file "$build_script"
...
GCC_END=$(date +%s)
trace_pkg_capture --pkg gcc-pass1 --version 15.2.0 --tier toolchain --phase all --rc 0 --duration-ms "$(( (GCC_END - GCC_START) * 1000 ))" --log "$GCC_LOG" --cmd-file /mnt/intergenos/packages/toolchain/gcc-pass1/build.sh
...
trace_pkg_capture --pkg libstdcpp --version 15.2.0 --tier toolchain --phase all --rc 0 --duration-ms "$(( (LIBSTDCPP_END - LIBSTDCPP_START) * 1000 ))" --log "$LIBSTDCPP_LOG" --cmd-file /mnt/intergenos/packages/toolchain/libstdcpp/build.sh
