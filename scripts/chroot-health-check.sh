...
# B10 USA-1: the /tmp/.X11-unix race fix lives at the systemd-unit-ordering
# level via the gdm.service.d/10-x11-unix-race.conf drop-in (shipped by the
# gdm package; covered by its verify_paths). The earlier /etc/tmpfiles.d/
# x11-unix.conf duplicate-override approach was retired per decision D6 of
# 2026-05-21 (composes-with-upstream not fight-it). Defensive
# coverage moved to packages/desktop/gdm/package.yml verify_paths where
# the build-time gate fails loudly if the drop-in isn't deployed.
