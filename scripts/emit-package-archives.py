    ...
    with open(manifest_path, "r") as f:
        for line in f:
            ...
            if section == "description" and line.strip():
                ...
                # A FILE LIST entry may carry a " sha256:<64 hex>"
                # annotation and a directory carries a trailing slash. Both
                # are part of the canonical manifest shape; taking the line
                # verbatim as a path produced an entry that resolves to
                # nothing in the chroot, and this function's caller drops
                # every such entry and then reports the package as having no
                # files at all. Strip the annotation anchored at end of line
                # (paths may contain spaces, so never split on whitespace)
                # and skip directory markers — the archiver adds parents
                # implicitly and an explicit shared-dir member would sweep in
                # other packages' content.
                entry = _SHA256_SUFFIX_RE.sub("", line.strip())
