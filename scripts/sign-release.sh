...
# Shared build-output library — for the ✓/✗/⚠ markers + TTY-aware color.
...
if [[ "$PKCS11_URI" == *"pin-value="* ]] || [[ "$PKCS11_URI" == *"pin-source="* ]]; then
    die "PKCS11_URI must not embed PIN material. Use canonical 'pkcs11:id=%02;type=private' and let the engine prompt." 2  # B-049
fi
...
if [[ ! -f "$PKCS11_MODULE" ]]; then
    die "patched OpenSC PKCS#11 module not found at $PKCS11_MODULE. See docs/signing-procedure.md." 2  # B-005
fi
...
if command -v pkcs11-tool >/dev/null 2>&1 && command -v openssl >/dev/null 2>&1; then
    ...
    if pkcs11-tool --module "$PKCS11_MODULE" --read-object --type cert --id 02 \
        ...
        if [[ "$_card_modulus" != "$_repo_modulus" ]]; then
            die "modulus mismatch: PIV slot 9c cert does not match $VENDOR_CERT. Signing would produce verify-broken artifacts." 2  # B-023
        fi
        echo "${IGOS_MARK_OK:-✓} modulus match: PIV slot 9c <-> $VENDOR_CERT"
    ...
    echo "warning: pkcs11-tool or openssl not available; skipping modulus guard" >&2  # B-023
fi
...
if [[ ${#unique_ukis[@]} -gt 0 ]]; then
    ...
    if (( signed_uki_count != ${#unique_ukis[@]} )); then
        die "post-sign UKI count mismatch: enumerated ${#unique_ukis[@]} input UKIs but signed $signed_uki_count" 3  # B-025
    fi
    echo "${IGOS_MARK_OK:-✓} UKI count: signed $signed_uki_count of ${#unique_ukis[@]} input"  # B-025
elif [[ "$STRICT" == "1" ]]; then
...
if (( ${#_in_efi[@]} != ${#_out_efi[@]} )); then
    ...
    die "post-sign .efi count mismatch: ${#_in_efi[@]} input(s) [$_in_names] vs ${#_out_efi[@]} output(s) [$_out_names]" 3  # B-025
fi
echo "${IGOS_MARK_OK:-✓} .efi count: ${#_out_efi[@]} signed = ${#_in_efi[@]} input"  # B-025

# -------- atomic promotion (SR1) --------
# S2-F2: the promotion is the step that DELIVERS the signed set — it must not
# swallow errors. A masked failure here leaves $OUTPUT holding the PRIOR run's
# validly-signed artifacts and the script reports success over a stale set
# (stale-input-that-verifies-anyway). Fail loud, then assert staging is empty
# before disarming the cleanup trap.
mv "$OUTPUT_STAGING"/* "$OUTPUT/" || die "promotion failed: mv $OUTPUT_STAGING/* -> $OUTPUT" 3  # S2-F2
_unmoved=$(ls -A "$OUTPUT_STAGING" 2>/dev/null || true)
[[ -z "$_unmoved" ]] || die "promotion incomplete — left in staging: $_unmoved" 3  # S2-F2
rmdir "$OUTPUT_STAGING" || die "promotion incomplete: staging dir not removable after move" 3  # S2-F2
trap - EXIT
