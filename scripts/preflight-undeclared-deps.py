...
Coverage classes (inability to inspect a selected package's source — these
FAIL CLOSED in gate mode, exit 3; --advisory restores the old skip):
  SOURCE-NOT-FOUND                a DECLARED source tarball is absent from
                                  the sources dir (staging gap)
  SOURCE-EXTRACT-FAILED           a scannable archive failed extraction
                                  (corrupt tarball, or a host tool such as
                                  lzip missing — the error names it)
  SCAN-ERROR                      per-package exception during the scan

By-design classes (always advisory — there is nothing to scan):
  NO-SOURCE-DECLARED              package.yml declares source: [] (files-only
                                  packages, download helpers, source_tree
                                  first-party builds)
  SOURCE-NOT-ARCHIVE              source[0] is not a tar-family/zip archive
                                  (.msi/.rpm/.pcf.gz binary payloads carry no
                                  buildfiles)

Exit codes:
  0 — clean (no HARD findings; in gate mode, additionally full coverage)
  1 — HARD findings present (build kickoff should halt)
  2 — environment problem (repo not found, unknown --only name, etc.)
  3 — gate mode: coverage failure — the sources dir is absent, or a selected
      package's declared source could not be located/extracted/scanned. A
      success exit must mean the intended set WAS inspected; "couldn't look"
      is not a pass. (--advisory keeps the historical skip-with-info exit.)

Usage:
  scripts/preflight-undeclared-deps.py             # gate mode (fail-closed)
  scripts/preflight-undeclared-deps.py --advisory  # exploratory: coverage
                                                   # gaps don't gate the exit
...
def _meson_required(arg_str: str) -> bool:
    ...
    """
    # A `fallback:` kwarg means meson satisfies the dependency from a
    # vendored subproject when the system dep is absent (and errors
    # LOUDLY if neither exists) — not a hard system requirement, so it
    # classifies SOFT. (First hit: dxvk's libdisplay-info, whose
    # subproject the recipe vendors as a Rule-5 source; 2026-07-03.)
    if re.search(r"fallback\s*:", arg_str):
        return False
...
def scan_package(pkg_name: str, pkg_info: dict, sources: Path, cache: Path,
    ...
    # By-design: no source at all (files-only packages, download helpers,
    # source_tree first-party builds) — there are no buildfiles to scan.
    # This must stay distinct from a DECLARED source that is absent, which
    # is a staging gap and gates in gate mode.
    if not (pkg_info.get("source") or []):
        return [{"type": "NO-SOURCE-DECLARED", "consumer": pkg_name}]

    tarball = find_source_tarball(sources, pkg_info)
    if tarball is None:
        return [{"type": "SOURCE-NOT-FOUND", "consumer": pkg_name}]

    # By-design: a non-archive source payload (.msi/.rpm/.pcf.gz etc.)
    # carries no buildfiles — extraction would be meaningless, not failed.
    if not _is_scannable_archive(tarball.name):
        return [{"type": "SOURCE-NOT-ARCHIVE", "consumer": pkg_name,
                 "tarball": str(tarball)}]
    ...
    for _bs, ftype, fdata in discoveries:
        ...
        # lib32 twin awareness (2026-07-03): a lib32-* consumer builds
        # -m32 against the 32-bit twins, so a requirement resolving to
        # package X is satisfied by a declared `lib32-X` (the provider
        # of X's 32-bit .pc/.so the twin actually links) or by being
        # X's own twin (lib32-X requiring X's module = itself). Without
        # this the gate flags every twin for deps it genuinely declares
        # (the 2026-07-03 GE-01 launch: 50+ false HARD findings on
        # correctly-declared twins).
        _is32 = pkg_name.startswith("lib32-")

        def _satisfied(c: str) -> bool:
            if c in declared or c == pkg_name:
                return True
            if _is32 and (f"lib32-{c}" in declared or c == pkg_name[6:]):
                return True
            return False
        ...
        if isinstance(resolved, list):
            candidates = resolved
            if any(_satisfied(c) for c in candidates):
                continue
...
def scan(repo: Path, sources: Path, cache: Path, aliases: dict,
    ...
    if only:
        unknown = sorted(set(only) - known_pkgs)
        if unknown:
            raise UnknownOnlyNames(unknown)
...
# Coverage classes: the scanner could NOT inspect a selected package's
# declared source. In gate mode (the default — the phase_validate call site
# and the targeted-resume manual fire) these FAIL CLOSED with exit 3: a
# success exit must mean the intended set was actually inspected. The
# by-design classes NO-SOURCE-DECLARED / SOURCE-NOT-ARCHIVE are deliberately
# NOT here — for those there is nothing to inspect.
COVERAGE_TYPES = frozenset({
    "SOURCE-NOT-FOUND",
    "SOURCE-EXTRACT-FAILED",
    "SCAN-ERROR",
})

# Archive shapes the scanner can open for buildfiles. Anything else that
# appears as source[0] is a binary/asset payload (SOURCE-NOT-ARCHIVE).
_SCANNABLE_SUFFIXES = (".tar.gz", ".tar.xz", ".tar.bz2", ".tar.lz",
                       ".tar.zst", ".tgz", ".zip")


def _is_scannable_archive(filename: str) -> bool:
    low = filename.lower()
    return any(low.endswith(s) for s in _SCANNABLE_SUFFIXES)


class UnknownOnlyNames(Exception):
    """--only listed a package name the tree does not have — a typo here
    previously reduced the scan to zero packages and exited 0."""

    def __init__(self, names: list[str]):
        super().__init__(", ".join(names))
        self.names = names

# Width the human-facing summary wraps prose to (the resolution guidance and
# any multi-line note). Fixed rather than terminal-derived so gate output is
# reproducible across a tty, a pipe, and a CI capture.
_OUTPUT_WIDTH = 78
...
def emit_summary(findings: list[dict], verbose: bool) -> None:
    ...
    # HARD categories stay itemized — they are the actionable, gate-blocking
    # ones. The INFO categories are collapsed to a single summary line (they
    # were a per-category wall); their per-category detail is carried by the
    # --report artifact and by --verbose (which lists every finding below).
    info_types = [t for t in sorted(by_type.keys()) if t not in HARD_TYPES]
    for t in sorted(by_type.keys()):
        if t not in HARD_TYPES:
            continue
        print(f"  [HARD] {t}: {len(by_type[t])}")
    if info_types:
        info_count = sum(len(by_type[t]) for t in info_types)
        find_word = "finding" if info_count == 1 else "findings"
        cat_word = "category" if len(info_types) == 1 else "categories"
        print(f"  [INFO] {info_count} {find_word} in {len(info_types)} "
              f"{cat_word} (detail: --report / --verbose)")
    ...
    if hard_count > 0:
        print()
        resolve_msg = (
            "FAIL — undeclared HARD build-deps detected. Resolve by adding "
        ...
        print(textwrap.fill(resolve_msg, width=_OUTPUT_WIDTH))
...
def write_artifacts(repo: Path, findings: list[dict], ts: str) -> tuple[Path, Path]:
    ...
    # Per-category breakdown carried here so the collapsed [INFO] summary line
    # in emit_summary has a home for its detail (the "route to --report" side
    # of the INFO-wall collapse). Purely a re-shaping of `findings`.
    by_type_counts: dict[str, int] = {}
    for f in findings:
        by_type_counts[f["type"]] = by_type_counts.get(f["type"], 0) + 1
    summary = {
        "total": len(findings),
        "hard": sum(c for t, c in by_type_counts.items() if t in HARD_TYPES),
        "by_type": {t: by_type_counts[t] for t in sorted(by_type_counts)},
    }
...
def main() -> int:
    ap = argparse.ArgumentParser(
        ...
        epilog="Exit 0 on clean, 1 on HARD findings, 2 on env problem "
               "(incl. unknown --only names), 3 in gate mode when coverage "
               "is incomplete (sources dir absent, or a declared source "
               "could not be located/extracted/scanned). --advisory keeps "
               "the historical skip-with-info exit for exploratory runs.",
    )
    ...
    ap.add_argument("--only", nargs="*", default=None,
                    help="restrict scan to listed package names "
                         "(unknown names are an error, exit 2)")
    ap.add_argument("--progress", action="store_true",
                    help="print progress to stderr every 25 packages")
    ap.add_argument("--advisory", action="store_true",
                    help="exploratory mode: coverage gaps (missing sources "
                         "dir, unlocatable/unextractable declared sources, "
                         "scan errors) are reported but do not gate the "
                         "exit. Gate mode (the default, used by "
                         "phase_validate and the targeted-resume manual "
                         "fire) fails closed on them.")
    args = ap.parse_args()
    ...
    if not sources.is_dir():
        if not args.advisory:
            print(f"FAIL-CLOSED: sources dir {sources} is absent — the "
                  f"scanner cannot inspect ANY package source, so a success "
                  f"exit here would certify an unexamined tree. Stage the "
                  f"sources (build pre-flight mandates it) or run with "
                  f"--advisory for an exploratory pass. Halting.",
                  file=sys.stderr)
            return 3
    ...
    except UnknownOnlyNames as e:
        print(f"ERROR: --only names not in the package tree: {e} — "
              f"a typo here would silently scan zero packages.",
              file=sys.stderr)
        return 2
    ...
    # Gate mode: an inability to inspect a selected package's declared
    # source fails closed. Previously these were advisory, so on a run
    # where sources were unstaged (or one package's tarball was corrupt /
    # needed a missing host tool) the gate exited 0 having inspected a
    # subset — success did not mean "audited."
    coverage = [f for f in findings if f["type"] in COVERAGE_TYPES]
    if coverage and not args.advisory:
        print("FAIL-CLOSED: incomplete scan coverage — these packages "
              "declare a source the scanner could not inspect:",
              file=sys.stderr)
        for f in coverage:
            detail = str(f.get("error", "")).strip().replace("\n", " ")
            tarball = f.get("tarball", "")
            extra = f" [{tarball}]" if tarball else ""
            extra += f" — {detail[:160]}" if detail else ""
            print(f"  - {f['type']}: {f['consumer']}{extra}", file=sys.stderr)
        print("Stage/repair the sources (or name the tool the error cites), "
              "or run with --advisory for an exploratory pass. Halting.",
              file=sys.stderr)
        return 3
    return 0
