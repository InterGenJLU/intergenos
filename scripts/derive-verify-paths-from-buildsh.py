...
def _kernel_kver() -> str:
    """Kernel KERNELRELEASE (<version>-igos-<release>) derived from linux-kernel's
    package.yml — INDEPENDENTLY of build.sh, so the kernel verify_paths are a real
    backstop against a mis-stamped kernel (WC review of 17875898), and so they stay
    correct across release bumps instead of a hardcoded literal. Fails LOUD on an
    unparseable version/release rather than emitting a guessed path (a silent
    default would make the pre-squashfs audit pass on the WRONG value)."""
    yml = REPO / 'packages' / 'core' / 'linux-kernel' / 'package.yml'
    ver = rel = None
    ...
    if not ver or not rel:
        raise SystemExit(
            f"FATAL: cannot parse version/release from {yml} — refusing to emit a "
            f"kernel verify_path on a guessed release (would mask a mis-stamp)")
    return f"{ver}-igos-{rel}"
        ...
        for suf in ['-pass1', '-pass2', '-bootstrap']:
            if name.endswith(suf):
                ...
                if base in ('linux-kernel',):
                    _kv = _kernel_kver()
                    return [f'/boot/vmlinuz-{_kv}', f'/usr/lib/modules/{_kv}'], 'heuristic'
                if base in ('gobject-introspection',):
                    return ['/usr/lib/libgirepository-1.0.so'], 'heuristic'
                return [f'/usr/bin/{base}'], 'heuristic'

    # linux-kernel: derive the release-stamped paths from package.yml (fail-loud),
    # NOT a hardcoded literal — keeps verify_paths a correct, independent backstop
    # against a mis-stamped kernel across release bumps. Handled before KNOWN so
    # _kernel_kver() runs only for the kernel, not on every package's derivation.
