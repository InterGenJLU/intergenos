...
#       [verdict_output_file]
#
# Artifact inventory (post-Dockerfile-revision, master 66c373e):
...
# objcopy with a single file argument REWRITES that file in place (the class
# that stripped the ge9b-01 UKI signatures, 2026-07-11). Mutating the work
# copies here would corrupt any later whole-file comparison of A/B — the
# explicit discard outputs keep both reads side-effect-free.
_SBAT_PE_DISCARD_A=$(mktemp); _SBAT_PE_DISCARD_B=$(mktemp)
SBAT_A_HASH=$(objcopy --dump-section .sbat=/dev/stdout "$WORK/A/shimx64.efi" "$_SBAT_PE_DISCARD_A" 2>/dev/null | sha256sum | awk '{print $1}')
SBAT_B_HASH=$(objcopy --dump-section .sbat=/dev/stdout "$WORK/B/shimx64.efi" "$_SBAT_PE_DISCARD_B" 2>/dev/null | sha256sum | awk '{print $1}')
rm -f "$_SBAT_PE_DISCARD_A" "$_SBAT_PE_DISCARD_B"
