...
if [ -d "$INTEL_SRC" ]; then
    ...
    # IMPORTANT: do NOT pass -S (--scan-system). -S filters microcode to ONLY
    # the signatures matching the RUNNING CPU (the build VM), which yields
    # an empty early-firmware image if the build VM's CPU signature isn't
    # in Intel's pack (which is the common case on cloud/virtualized hosts).
    # The early-firmware image we produce here ships to end-user hardware
    # of varied vintages — we want the FULL pack, kernel selects the
    # matching signature at boot. Without -S, iucode_tool reads every
    # microcode file in the source directory and writes them all.
    if "$TOOL" "$INTEL_SRC/" --write-earlyfw="$OUTPUT_DIR/intel-ucode.img" 2>/dev/null; then
        if [ -s "$OUTPUT_DIR/intel-ucode.img" ]; then
            echo "intel-ucode.img written ($(stat -c %s "$OUTPUT_DIR/intel-ucode.img") bytes)"
        else
            # iucode_tool exited 0 but produced empty output — should not
            # happen with -S removed unless $INTEL_SRC is genuinely empty.
            rm -f "$OUTPUT_DIR/intel-ucode.img"
            echo "intel-ucode.img skipped (iucode_tool produced empty output)"
        fi
    else
