...
if [ "${IGOS_TRACE_LIB_LOADED:-0}" = "1" ]; then
    trace_init "host-build-uki"
    _UKI_START_MS=$(date +%s%3N)
    trace_event uki_assemble_start \
        output_path="$OUTPUT" vmlinuz="$VMLINUZ" initramfs="$INITRAMFS" \
        cmdline="$CMDLINE" os_release="$OS_RELEASE" stub="$STUB" \
        microcode="$MICROCODE"
fi
...
_UKIFY_RC=$?
if [ "${IGOS_TRACE_LIB_LOADED:-0}" = "1" ]; then
    trace_event ukify_invoke \
        ukify="$UKIFY" output="$OUTPUT" kver="$KVER" \
        rc::=$_UKIFY_RC duration_ms::=$(( $(date +%s%3N) - _UKIFY_START_MS ))
fi
...
if [ "${IGOS_TRACE_LIB_LOADED:-0}" = "1" ]; then
    trace_event uki_assemble_end \
        output_path="$OUTPUT" size_bytes::=$UKI_SIZE \
        sha="$(sha256sum "$OUTPUT" | awk '{print $1}')" kver="$KVER" \
        rc::=0 duration_ms::=$(( $(date +%s%3N) - _UKI_START_MS ))
    trace_close
fi
