...
MODULES=(
    ...
    # Pattern-match in grub-script.  Needed by /EFI/InterGenOS/grub.cfg
    # to extract the WxH prefix from $gfxmode (which GRUB sets as either
    # "WxH" or "WxHxC" depending on the firmware/driver path: vbe / efifb
    # / gop). Without regexp the resolution-specific PNG lookup fails on
    # the WxHxC form because we ship PNGs named WxH only — operator
    # surfaced this on the laptop boot 2026-05-26. Matches the installed-
    # system mechanism in /etc/grub.d/06_intergenos_multi_background
    # (which depends on regexp via grub-mkconfig_lib).
    regexp

    # Modules the GENERATED /etc/grub.d cfg on an installed system insmods
    # at boot. Under Secure Boot the built-in shim_lock verifier refuses any
    # module loaded as a loose file from the ESP ("prohibited by secure boot
    # policy", kern/efi/sb.c) — each name missing here printed that error on
    # the boot console at every menu display (visible on the second monitor
    # of a multi-head system, where the un-themed console renders). Baking
    # them in makes them signature-covered instead of unsigned ESP files:
    #   efi_uga  — 00_header load_video fallback (read-only display proto)
    #   gettext  — 00_header menu translations (read-only)
    #   gzio     — 10_linux gzip'd kernel/initramfs read; REQUIRED for the
    #              fallback/recovery entries to load the gzip'd initramfs
    #   bli      — 25_bli Boot Loader Interface EFI vars; the same vars the
    #              UKI stub sets on the primary path, kept consistent on the
    #              fallback path (spec-defined identification vars, not the
    #              loadenv/savedefault env-write-to-disk exclusion class)
    efi_uga gettext gzio bli
)
