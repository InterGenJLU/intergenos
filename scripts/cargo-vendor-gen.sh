...
usage() {
    cat >&2 <<EOF
Usage: ${SCRIPT_NAME} [--cargo-lock <path>] [--subdir <rel>] <pkg-name> <version> <source-url-or-path>
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        --cargo-lock)
            ...
            ;;
        --subdir)
            # Workspace/binding layouts (maturin sdists, setuptools-rust) keep
            # the cargo manifest in a subdirectory (tokenizers: bindings/python,
            # hf-xet: hf_xet, libcst: native). Vendoring runs THERE; the wrapper
            # tarball still stages vendor/ + .cargo/ at the SDIST ROOT level so
            # every consumer build.sh keeps the same --strip-components=1
            # extraction, and cargo's ancestor config-walk resolves
            # root/.cargo/config.toml (directory = "vendor" is root-relative).
            [ "$#" -ge 2 ] || die "--subdir requires a <relative-path> argument"
            SUBDIR="$2"
            case "$SUBDIR" in /*|*..*) die "--subdir must be a clean relative path" ;; esac
            shift 2
            ;;
        --help|-h)
            usage
...
MANIFEST_DIR="$PROJECT_ROOT${SUBDIR:+/$SUBDIR}"
[ -d "$MANIFEST_DIR" ] || die "--subdir path not found in source: $SUBDIR"
[ -f "$MANIFEST_DIR/Cargo.toml" ] || die "no Cargo.toml at ${SUBDIR:-project root} — not a cargo project"
[ -n "$SUBDIR" ] && log "manifest dir (--subdir): $MANIFEST_DIR"
...
if [ -n "$CARGO_LOCK_PATH" ]; then
    ...
    (cd "$MANIFEST_DIR" && cargo generate-lockfile) >&2 \
...
CARGO_DIR="$MANIFEST_DIR/.cargo"
mkdir -p "$CARGO_DIR"
(cd "$MANIFEST_DIR" && cargo vendor --locked --versioned-dirs > "$CARGO_DIR/config.toml") \
    || die "cargo vendor failed (Cargo.lock stale, or crates.io unreachable)"

VENDOR_DIR="$MANIFEST_DIR/vendor"
