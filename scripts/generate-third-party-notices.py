...
def main(check_only: bool = False) -> int:
    ...
    if check_only:
        # Gate mode (decided 2026-08-16): regenerate in memory and compare
        # against the committed file. Two independent correct recipe fixes
        # once left the published attribution stale for eleven days because
        # nothing re-derived this file; the pre-push hook now refuses that
        # class. Exit 0 in sync, 2 on drift, 1 on error.
        try:
            committed = OUTPUT.read_text()
        except OSError as exc:
            sys.stderr.write(f"ERROR: cannot read {OUTPUT}: {exc}\n")
            return 1
        if committed == text:
            print(f"[third-party-notices] in sync ({total} packages).")
            return 0
