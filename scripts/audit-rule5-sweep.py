...
Writes the rule5 classifier output to two locations:
  /tmp/rule5-sweep-results.tsv         (short-lived debug artifact)
  <canonical-output>/audits/rule5-sweep-results.tsv

The canonical output location resolves from $INTERGENOS_AUDITS_DIR/.. or
$INTERGENOS_PRIVATE_REPO/audits or sibling intergenos-private/audits
(fallback to public build/ with stderr warning). Per decided
2026-05-29, classifier output lives in the private project repo, not in
...
def _resolve_audits_dir():
    ...
    Mirrors audit-package.py + aggregate-package-audits.py. Resolution
    order:
      ...
      4. <REPO>/build/audits — public-tree fallback with warning.
    """
...
def main():
    ...
    # Canonical output lands beside the per-package JSONs (one level up,
    # in audits/ root). AUDITS = .../audits/per-package; .parent = audits/.
    out_canonical = AUDITS.parent / "rule5-sweep-results.tsv"
    for out_path in (out_tmp, out_canonical):
