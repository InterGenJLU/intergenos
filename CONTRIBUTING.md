...
Thanks for your interest in InterGenOS. The project is in active development,
currently at version 1.0-dev. It is a Linux distribution built entirely from
source, with a custom package manager (`pkm`), the Forge installer, a mandatory
Secure Boot chain, and a tiered local AI assistant (InterGen). Today it ships
GNOME 49 on Wayland; contributions of all kinds are welcome.
...
- **How does the package manager work?** Read the [pkm internals documentation](docs/components/pkm.md).
...
The most impactful contributions are package templates. Each package lives under
`packages/<tier>/<name>/` and needs two files:
...
internal development artifacts — host-specific paths, internal file references,
and credential-like strings — do not enter the public repository. If the audit
fails on your PR, amend the flagged files and push again. Legitimate public
content that happens to match a flagged pattern can be added to
`scripts/check-public-content.allowlist`.

### What We Value

- **Research before implementing** — check the LFS/BLFS book, and study how other distributions handle it
...
See the [README](README.md) for a build system overview. The build requires:
