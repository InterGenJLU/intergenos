...
Both versions read as "heartbeat pulse", so the mark stays recognisable across
the size range — the standard approach for logos that scale from favicons to
high-res displays.

Logo PNGs are rendered at 512, 1024, 1536, 2048 px wide.

**Wordmark / full lockup (NOT generated by `generate.py`, hand-finished):**

| File | Dimensions | Backdrop | Origin |
|------|------------|----------|--------|
| `intergenos_wordmark.png` | 1751×898 | Pure black `#000000` | Externally rendered (designer-finished) — pulse on left, `INTERGENOS` wordmark in restrained wide-tracked thin uppercase along the trail, "SECURITY IS NOT FIRST. IT IS ONLY." tagline below with `ONLY` emphasized, ECG-blue glow underline running the full width |
| `intergenos_wordmark_transparent.png` | 1751×898 | Transparent (RGBA) | GIMP `color-to-alpha` against `#000000` on the black-bg source, then antialiasing + sharpening pass to recover tagline edge crispness |

The wordmark is the canonical brand lockup for use on GDM login screen, intergen-welcome
hero, About dialog, README screenshots, and any other "full brand identity" surface where
both the pulse mark and the InterGenOS name need to read together. For favicon-class
surfaces (window-list, dock, application launcher), use the square `intergenos_icon_*`
family instead.
