...
# Rounded-corner radius for the square 512x512 badge tile (≈21%, GNOME-style
# squircle). Applied to the black-bg icon variants only — the wide logo
# lockup keeps hard corners. Operator branding pass 2026-06-11 (§B): the
# forge "Install InterGenOS" tile had stark hard corners.
ICON_CORNER_RADIUS = 110
...
def make_svg(viewbox, path_d, stroke, stroke_color=BLUE, bg=None, corner_radius=None):
    vb_x, vb_y, vb_w, vb_h = viewbox
    parts = [f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="{vb_x} {vb_y} {vb_w} {vb_h}">']
    if bg is not None:
        # corner_radius rounds the badge tile (GNOME-style squircle). Only the
        # square icon variants pass it; the wide logo lockup keeps hard corners.
        rxy = f' rx="{corner_radius}" ry="{corner_radius}"' if corner_radius else ''
        parts.append(f'<rect x="{vb_x}" y="{vb_y}" width="{vb_w}" height="{vb_h}"{rxy} fill="{bg}"/>')
    parts.append(
...
# Operator-authored / externally-produced PNGs that live in png/ but are NOT
# regenerated by this script (the Forge hero banner + the wordmark crops).
# The clean-slate step MUST preserve these — a blind rmtree(PNG_DIR) silently
# destroys them and the intergen-mark build.sh then FATALs on the missing
# /usr/share/intergenos/intergenos_{pulse_forge_hero,wordmark*}.png assets.
PRESERVE_PNG = {
    "intergenos_pulse_forge_hero.png",
    "intergenos_wordmark.png",
    "intergenos_wordmark_header.png",
    "intergenos_wordmark_transparent.png",
}


def generate():
    # Clean slate. SVG_DIR is wholly owned by this script (every .svg in it is
    # regenerated below), so a full wipe is safe. PNG_DIR also holds the
    # foreign assets in PRESERVE_PNG, so wipe it file-by-file instead, skipping
    # those.
    if SVG_DIR.exists():
    ...
    for f in PNG_DIR.iterdir():
        if f.is_file() and f.name not in PRESERVE_PNG:
            f.unlink()
