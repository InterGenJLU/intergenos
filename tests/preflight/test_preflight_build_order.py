...
class TestScanCompleteness(unittest.TestCase):
    """Review finding M1: missing phase drivers are NAMED, and a scan that
    parsed zero packages FAILS instead of passing vacuously."""
    ...
    def test_empty_scan_fails(self):
        # No phase driver exists at all: the old code passed ("no findings").
        with tempfile.TemporaryDirectory() as td:
            repo = _make_repo(Path(td), packages={}, scripts={})
            rc, out = self._run(repo)
            self.assertEqual(rc, 1)
            self.assertIn("EMPTY-SCAN", out)

    def test_missing_driver_is_named(self):
        with tempfile.TemporaryDirectory() as td:
            repo = _make_repo(
                Path(td),
                packages={"pkg-a": {"tier": "core"}},
                scripts={"ch8": ["pkg-a"]})
            rc, out = self._run(repo)
            self.assertEqual(rc, 0, out)
            self.assertIn("Phase drivers absent (named skip):", out)
            self.assertIn("chroot-build-base.sh", out)
