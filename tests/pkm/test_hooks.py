...
class TestRunCanonicalHooks(unittest.TestCase):
    ...
    def test_cosmetic_hook_failure_flags_cosmetic_not_critical(self):
        ...
        # PI-13 (4e25d6ab): the icon-cache hook only fires for a theme dir that
        # already has an index.theme — otherwise it skips (that dir gets cached
        # when its owning theme package lands). The fixture must therefore seed
        # index.theme so the cosmetic-failure path is actually exercised.
        theme_dir = self.root / "usr/share/icons/InterGenOS"
        (theme_dir / "16x16").mkdir(parents=True)
    ...
    def test_icon_cache_skips_theme_dir_without_index_theme(self):
        # PI-13 (4e25d6ab): a theme dir lacking index.theme is skipped, NOT run
        # — during a fresh install icon-shipping packages are processed before
        # hicolor-icon-theme lands its index.theme, and running the cache there
        # floods the trace with expected "No theme index file" failures that
        # could mask a real one. No index.theme → hook does not fire → no
        # failure of either class, and the stub is never invoked.
        _, log, _ = _make_fake_bin(self.bin, "gtk-update-icon-cache", exit_code=1)
        file_list = ["usr/share/icons/hicolor/16x16/apps/foo.png"]
        result = run_canonical_hooks(self.root, file_list, "foo", "1.0", "install")
        self.assertEqual(result.critical_failures, [])
        self.assertEqual(result.cosmetic_failures, [])
    ...
    def test_apparmor_reload_passes_profile_paths(self):
        # apparmor_parser -r loads profiles into the RUNNING kernel, so it is
        # built ONLY for a live-system target (root == "/") WITH a live
        # AppArmor LSM interface; both are pinned here so the test does not
        # inherit the test host's kernel config. -r + the absolute paths.
        from unittest import mock
        with mock.patch.object(hooks_mod, "_APPARMOR_IFACE",
                               Path(self.root)):  # exists -> interface live
    ...
    def test_apparmor_reload_skipped_without_lsm_interface(self):
        # A live root whose kernel exposes no AppArmor interface (a build
        # chroot on a host without securityfs/apparmor, or AppArmor disabled)
        # has nothing to load profiles into — the hook must SKIP, not fail:
        # an in-chroot redeploy marked apparmor DEGRADED in the build
        # database on 2026-07-30, false factory metadata on a correct image.
        # apparmor.service loads the profiles at the next boot. An absent
        # interface is the ONLY thing this skip covers — a present interface
        # with a failing parser stays CRITICAL.
        from unittest import mock
        with mock.patch.object(hooks_mod, "_APPARMOR_IFACE",
                               Path(self.root) / "no-such-interface"):
            cmd = _apparmor_parser_cmd("/", ["etc/apparmor.d/usr.bin.foo"])
        self.assertIsNone(cmd)

    def test_apparmor_reload_skipped_for_chroot_target(self):
        # For a chroot/staging install (root != "/") apparmor_parser -r would
        # load the target's profiles into the LIVE kernel against the wrong
        # binary paths (install #17, 2026-05-26). The hook must NOT fire; the
        # target's apparmor.service loads them at first boot. Asserting the
        # skip — not a defect — is the correct contract (mirrors
        # test_systemd_daemon_reload_skipped_for_non_root_target).
        ...
        self.assertEqual(result.critical_failures, [])
        self.assertEqual(result.cosmetic_failures, [])
        self.assertFalse(log.exists(), "apparmor_parser ran in chroot context")
        self.assertIsNone(_apparmor_parser_cmd(self.root, file_list))
