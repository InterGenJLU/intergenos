...
class HelperLibPartialManifestEndToEndTests(unittest.TestCase):
    ...
    def _run(self, script_body):
        ...
        """
        script_body = script_body.replace("{dep}", f"{self.tmp}/deposits")
        script = f"""
set +e
...
mkdir -p {dep}/crashpkg/lib
printf x > {dep}/crashpkg/bin
printf x > {dep}/crashpkg/lib/libfoo.so
igos_helper_record_file {dep}/crashpkg/bin
igos_helper_record_file {dep}/crashpkg/lib/libfoo.so
...
"""
        ...
        self.assertEqual(
            data["files"],
            [f"{self.tmp}/deposits/crashpkg/bin",
             f"{self.tmp}/deposits/crashpkg/lib/libfoo.so"],
        )
...
mkdir -p {dep}/okpkg && printf x > {dep}/okpkg/bin
igos_helper_record_file {dep}/okpkg/bin
igos_helper_record_dep glibc
igos_helper_commit
"""
        proc = self._run(body)
        ...
        self.assertEqual(data["files"], [f"{self.tmp}/deposits/okpkg/bin"])
...
igos_helper_set_version "1.0.0"
mkdir -p {{dep}}/ucleanup-crashpkg && printf x > {{dep}}/ucleanup-crashpkg/bin
igos_helper_record_file {{dep}}/ucleanup-crashpkg/bin
exit 1
"""
        proc = self._run(body)
        self.assertEqual(proc.returncode, 1)
...
igos_helper_set_version "2.0.0"
mkdir -p {{dep}}/ucleanup-okpkg && printf x > {{dep}}/ucleanup-okpkg/bin
igos_helper_record_file {{dep}}/ucleanup-okpkg/bin
igos_helper_commit
"""
        proc = self._run(body)
        self.assertEqual(proc.returncode, 0, msg=proc.stderr)
...
igos_helper_set_version "1.0.0"
mkdir -p {dep}/retrypkg && printf x > {dep}/retrypkg/old-bin
igos_helper_record_file {dep}/retrypkg/old-bin
exit 1
"""
        success_body = """
igos_helper_init "retrypkg"
igos_helper_set_version "1.0.1"
mkdir -p {dep}/retrypkg && printf x > {dep}/retrypkg/new-bin
igos_helper_record_file {dep}/retrypkg/new-bin
igos_helper_commit
"""
        ...
        data = json.loads(canonical.read_text())
        self.assertEqual(data["files"], [f"{self.tmp}/deposits/retrypkg/new-bin"])
        self.assertEqual(data["version_installed"], "1.0.1")

    def test_record_file_refuses_embedded_newline(self):
        # The staging record is line-oriented: a path with an embedded
        # newline would silently split into two bogus manifest entries
        # (the deposited file untracked, two phantom rows tracked).
        # record_file must refuse loudly instead.
        body = """
igos_helper_init "nlpkg"
igos_helper_set_version "1.0.0"
mkdir -p {dep}/nlpkg
p="{dep}/nlpkg/evil"$'\\n'"name"
printf x > "$p"
if igos_helper_record_file "$p"; then
    exit 0
fi
exit 42
"""
        ...
        self.assertEqual(
            proc.returncode, 42,
            f"record_file must refuse a newline-embedded path: {proc.stderr}",
        )
        self.assertIn("embedded newline", proc.stderr)
        # The refused path must not have produced any staged rows: a
        # crash sidecar for this run must carry an EMPTY files list.
        partial = self.manifest_dir / "nlpkg.manifest.partial"
        self.assertTrue(partial.is_file())
        data = json.loads(partial.read_text())
        self.assertEqual(data["files"], [])

    def test_record_symlink_refuses_embedded_tab_or_newline(self):
        # The symlink record is tab-separated + line-oriented; an
        # embedded tab or newline in either field corrupts the split.
        body = """
igos_helper_init "tabpkg"
igos_helper_set_version "1.0.0"
if igos_helper_record_symlink "/usr/bin/evil"$'\\t'"link" "/opt/target"; then
    exit 0
fi
exit 42
"""
        ...
        self.assertEqual(
            proc.returncode, 42,
            f"record_symlink must refuse a tab-embedded link_path: {proc.stderr}",
        )
        # igos_helper_emit line-wraps long messages — assert a phrase
        # that cannot straddle the wrap point.
        self.assertIn("record_symlink", proc.stderr)
        self.assertIn("embedded newline", proc.stderr)
