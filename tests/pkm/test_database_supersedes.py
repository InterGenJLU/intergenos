...
from pkm.database import (  # noqa: E402
    PATH_ABSENT,
    PATH_PRESENT,
    PackageDB,
    _parse_manifest,
...
class VerifyPackageStrictModeTests(unittest.TestCase):
    """verify_package strict vs fast — modes drive hash compare.

    These patch _probe_path, the seam that answers present / absent /
    undeterminable for one path. They used to patch os.path.lexists; that
    call was replaced because it cannot distinguish "not there" from "not
    allowed to look", which made a non-root verify report readable-only-by-root
    files as missing.
    """
