...
def _import_pubkey_to_keyring(gnupghome, target_keyring):
    """Materialize `target_keyring` (the gpgv keyring) by EXPORTING the pubkey
    straight to the file as binary key material — NOT via `gpg --keyring
    --import`.

    On gpg 2.5.x with use-keyboxd (default-on in the gpg InterGenOS ships),
    `gpg --keyring <file> --import` routes the key into keyboxd's own store and
    writes NOTHING to the legacy --keyring file, leaving gpgv an empty keyring
    (-> no VALIDSIG -> a good signature wrongly rejected, which is the development machine
    green-path failure). `gpg --export` is keyboxd-agnostic and gpgv reads the
    raw concatenated-pubkey file directly — exactly how the real
    /etc/pkm/trusted.gpg keyring is materialized. Portable across gpg 2.4.x
    (keyboxd off) and 2.5.x (keyboxd default-on).
    """
    result = subprocess.run(
        ["gpg", "--homedir", str(gnupghome), "--batch", "--export"],
        capture_output=True, check=True,  # binary stdout — no text=True
    )
    Path(target_keyring).write_bytes(result.stdout)
