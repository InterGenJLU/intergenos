...
class TestCheckUpdatesVersionParseError(unittest.TestCase):
    ...
    def test_bad_version_skipped_others_processed(self):
        # An UNORDERABLE version on bad-pkg shouldn't block firefox from being
        # checked. Post PKM-A01 the dpkg comparator orders any non-empty string,
        # so the genuine unorderable case is an empty/missing version — that is
        # what still raises VersionParseError; cmd_check_updates WARN-and-skips
        # it. (A non-PEP-440 string like '140.9.0esr' is NOT bad and is no
        # longer skipped — that was the bug.)
        _add_installed(self.db, "bad-pkg", "138.0")
        _add_installed(self.db, "firefox", "138.0")
        remotes = {
            "bad-pkg": {"name": "bad-pkg", "version": "", "release": 1},
    ...
    def test_skipped_field_records_unevaluated_pkgs_even_quiet(self):
        # PKM-A15: a package that can't be version-compared must be recorded
        # in the JSON `skipped` array — written even under --quiet (the
        # unattended timer path), so the notification readers can surface
        # "K could not be evaluated" instead of silently under-reporting.
        _add_installed(self.db, "bad-pkg", "138.0")
        _add_installed(self.db, "firefox", "138.0")
        remotes = {
            "bad-pkg": {"name": "bad-pkg", "version": "", "release": 1},
        ...
        with patch("pkm.cli.RepoManager", return_value=_FakeRepoManager(remotes)):
            cmd_check_updates(self.db, _make_args(quiet=True),
                              output_path=self.out_path)
        data = json.loads(self.out_path.read_text())
        # The skipped package is NOT in packages, but IS in skipped.
        self.assertNotIn("bad-pkg", [p["name"] for p in data["packages"]])
        self.assertEqual(data["skipped_count"], 1)
        self.assertEqual(len(data["skipped"]), 1)
        self.assertEqual(data["skipped"][0]["name"], "bad-pkg")
        self.assertIn("version comparison failed", data["skipped"][0]["reason"])

    def test_skipped_empty_when_all_evaluable(self):
        _add_installed(self.db, "firefox", "138.0")
        remotes = {"firefox": {"name": "firefox", "version": "139.0",
                               "release": 1}}
        with patch("pkm.cli.RepoManager", return_value=_FakeRepoManager(remotes)):
            cmd_check_updates(self.db, _make_args(quiet=True),
                              output_path=self.out_path)
