...
# from the worktree THIS test file lives in, never a hardcoded absolute
# root (review finding M3: the hardcoded master-worktree path made the dev
# tree's test run silently exercise the other worktree's module).
...
def _make_pkg(name="testpkg", version="1.0"):
    return make_package(name=name, version=version)
